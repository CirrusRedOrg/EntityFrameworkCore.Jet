...
using EntityFrameworkCore.LibRed.Extended.FunctionalTests.Query;
...
public class NorthwindEFPropertyIncludeQueryLibRedTest : NorthwindEFPropertyIncludeQueryTestBase<
    NorthwindQueryLibRedFixture<NoopModelCustomizer>>
{
    public NorthwindEFPropertyIncludeQueryLibRedTest(NorthwindQueryLibRedFixture<NoopModelCustomizer> fixture)
        : base(fixture)
    {
...
ORDER BY `c2`.`CustomerID`, `s`.`OrderID`, `s`.`OrderID0`, `s`.`ProductID`
""");
    }
...
LEFT JOIN `Orders` AS `o` ON `c`.`CustomerID` = `o`.`CustomerID`
WHERE `c`.`CustomerID` LIKE 'F%'
ORDER BY IIF(`c`.`CustomerID` LIKE 'S%', 1, 2), `c`.`CustomerID`, `o`.`OrderID`
""");
    }
