...
public class NorthwindSplitIncludeQueryLibRedTest : NorthwindSplitIncludeQueryTestBase<
    NorthwindQueryLibRedFixture<NoopModelCustomizer>>
{
    // ReSharper disable once UnusedParameter.Local
    public NorthwindSplitIncludeQueryLibRedTest(
        NorthwindQueryLibRedFixture<NoopModelCustomizer> fixture,
...
""",
            //
            """
SELECT `o1`.`OrderID`, `o1`.`CustomerID`, `o1`.`EmployeeID`, `o1`.`OrderDate`, `o`.`OrderID`, `o`.`ProductID`
FROM ((`Order Details` AS `o`
INNER JOIN `Orders` AS `o0` ON `o`.`OrderID` = `o0`.`OrderID`)
LEFT JOIN `Customers` AS `c` ON `o0`.`CustomerID` = `c`.`CustomerID`)
LEFT JOIN `Orders` AS `o1` ON `c`.`CustomerID` = `o1`.`CustomerID`
WHERE ((`o`.`OrderID` MOD 23) = 13 AND `o`.`UnitPrice` < 10.0) AND (`c`.`CustomerID` IS NOT NULL AND `o1`.`CustomerID` IS NOT NULL)
ORDER BY `o`.`OrderID`, `o`.`ProductID`
""");
    }
...
""",
            //
            """
SELECT `o0`.`OrderID`, `o0`.`CustomerID`, `o0`.`EmployeeID`, `o0`.`OrderDate`, `s`.`OrderID`
FROM (
...
ORDER BY `c1`.`c` DESC, `c1`.`CustomerID`
""",
            //
            """
...
FROM (
    ...
    ORDER BY `c2`.`c` DESC, `c2`.`CustomerID`
) AS `c0`
...
WHERE (`c`.`CustomerID` LIKE 'F%') AND (`o`.`OrderID` IS NOT NULL AND `s`.`OrderID` IS NOT NULL)
ORDER BY `c`.`CustomerID`, `o`.`OrderID`
""");
    }
...
(
    SELECT TOP 2 `c3`.`CustomerID`, `c3`.`Address`, `c3`.`City`, `c3`.`CompanyName`, `c3`.`ContactName`, `c3`.`ContactTitle`, `c3`.`Country`, `c3`.`Fax`, `c3`.`Phone`, `c3`.`PostalCode`, `c3`.`Region`
    FROM (
        SELECT TOP 2 + 2 `c0`.`CustomerID`, `c0`.`Address`, `c0`.`City`, `c0`.`CompanyName`, `c0`.`ContactName`, `c0`.`ContactTitle`, `c0`.`Country`, `c0`.`Fax`, `c0`.`Phone`, `c0`.`PostalCode`, `c0`.`Region`
        FROM `Customers` AS `c0`
        ORDER BY `c0`.`CustomerID`
    ) AS `c3`
    ORDER BY `c3`.`CustomerID` DESC
) AS `c2`
ORDER BY `c1`.`CustomerID`, `c2`.`CustomerID`
""",
            //
...
INNER JOIN `Orders` AS `o` ON `c1`.`CustomerID` = `o`.`CustomerID`),
(
    SELECT TOP 2 `c3`.`CustomerID`
    FROM (
        SELECT TOP 2 + 2 `c0`.`CustomerID`
        FROM `Customers` AS `c0`
        ORDER BY `c0`.`CustomerID`
    ) AS `c3`
    ORDER BY `c3`.`CustomerID` DESC
...
SELECT TOP @p2 `c1`.`CustomerID`, `c1`.`Address`, `c1`.`City`, `c1`.`CompanyName`, `c1`.`ContactName`, `c1`.`ContactTitle`, `c1`.`Country`, `c1`.`Fax`, `c1`.`Phone`, `c1`.`PostalCode`, `c1`.`Region`, `c2`.`CustomerID`, `c2`.`Address`, `c2`.`City`, `c2`.`CompanyName`, `c2`.`ContactName`, `c2`.`ContactTitle`, `c2`.`Country`, `c2`.`Fax`, `c2`.`Phone`, `c2`.`PostalCode`, `c2`.`Region`
FROM (
    SELECT TOP @p `c`.`CustomerID`, `c`.`Address`, `c`.`City`, `c`.`CompanyName`, `c`.`ContactName`, `c`.`ContactTitle`, `c`.`Country`, `c`.`Fax`, `c`.`Phone`, `c`.`PostalCode`, `c`.`Region`
    FROM `Customers` AS `c`
    ORDER BY `c`.`CustomerID`
...
FROM (
    SELECT TOP @p2 `c1`.`CustomerID`, `c2`.`CustomerID` AS `CustomerID0`
    FROM (
        SELECT TOP @p `c`.`CustomerID`
        FROM `Customers` AS `c`
        ORDER BY `c`.`CustomerID`
...
FROM `Orders` AS `o`
LEFT JOIN `Customers` AS `c` ON `o`.`CustomerID` = `c`.`CustomerID`
ORDER BY NOT (`o`.`OrderID` > 0), IIF(`c`.`CustomerID` IS NOT NULL, `c`.`City`, ''), `o`.`OrderID`
""",
            //
            """
@p='5'

SELECT `o0`.`OrderID`, `o0`.`ProductID`, `o0`.`Discount`, `o0`.`Quantity`, `o0`.`UnitPrice`, `s`.`OrderID`
FROM (
    SELECT TOP @p `s0`.`OrderID`, `s0`.`c`, `s0`.`c0`
    FROM (
        SELECT `o`.`OrderID`, `o`.`OrderID` > 0 AS `c`, IIF(`c`.`CustomerID` IS NOT NULL, `c`.`City`, '') AS `c0`
        FROM `Orders` AS `o`
        LEFT JOIN `Customers` AS `c` ON `o`.`CustomerID` = `c`.`CustomerID`
    ) AS `s0`
    ORDER BY NOT (`s0`.`c`), `s0`.`c0`, `s0`.`OrderID`
) AS `s`
INNER JOIN `Order Details` AS `o0` ON `s`.`OrderID` = `o0`.`OrderID`
ORDER BY NOT (`s`.`c`), `s`.`c0`, `s`.`OrderID`
""");
    }
...
FROM ((
    SELECT TOP 1 `c3`.`CustomerID`, `c3`.`c`
    FROM (
        SELECT `c`.`CustomerID`, (
            SELECT TOP 1 `o`.`OrderDate`
            FROM `Orders` AS `o`
    ...
    ORDER BY `c3`.`c` DESC, `c3`.`CustomerID`
) AS `c0`
INNER JOIN `Orders` AS `o0` ON `c0`.`CustomerID` = `o0`.`CustomerID`)
...
INNER JOIN `Orders` AS `o3` ON `c1`.`CustomerID` = `o3`.`CustomerID`)
LEFT JOIN `Order Details` AS `o4` ON `o3`.`OrderID` = `o4`.`OrderID`
WHERE `o3`.`OrderID` IS NOT NULL AND `o4`.`OrderID` IS NOT NULL
ORDER BY `c1`.`CustomerID`, `o3`.`OrderID`
""");
    }
...
SELECT `o3`.`OrderID`, `o3`.`CustomerID`, `o3`.`EmployeeID`, `o3`.`OrderDate`, `c2`.`CustomerID`
FROM (
    SELECT TOP 1 `c0`.`CustomerID`
    FROM (
        SELECT TOP @p `c1`.`CustomerID`
...
INNER JOIN `Orders` AS `o3` ON `c2`.`CustomerID` = `o3`.`CustomerID`
ORDER BY `c2`.`CustomerID`, `o3`.`OrderID`
""",
            //
            """
@p='1'

SELECT `o4`.`OrderID`, `o4`.`ProductID`, `o4`.`Discount`, `o4`.`Quantity`, `o4`.`UnitPrice`, `c2`.`CustomerID`, `o3`.`OrderID`
FROM ((
    SELECT TOP 1 `c0`.`CustomerID`
    FROM (
        SELECT TOP @p `c1`.`CustomerID`
...
LEFT JOIN `Orders` AS `o` ON `c`.`CustomerID` = `o`.`CustomerID`)
LEFT JOIN `Order Details` AS `o0` ON `o`.`OrderID` = `o0`.`OrderID`
WHERE (`c`.`City` = 'Seattle') AND (`o`.`OrderID` IS NOT NULL AND `o0`.`OrderID` IS NOT NULL)
ORDER BY `c`.`CustomerID`, `o`.`OrderID`
""");
    }
...
LEFT JOIN `Customers` AS `c` ON `o0`.`CustomerID` = `c`.`CustomerID`)
LEFT JOIN `Orders` AS `o1` ON `c`.`CustomerID` = `o1`.`CustomerID`
WHERE ((`o`.`OrderID` MOD 23) = 13) AND (`c`.`CustomerID` IS NOT NULL AND `o1`.`CustomerID` IS NOT NULL)
ORDER BY `o`.`OrderID`, `o`.`ProductID`
""");
    }
...
""",
            //
            """
SELECT `o0`.`OrderID`, `o0`.`CustomerID`, `o0`.`EmployeeID`, `o0`.`OrderDate`, `o`.`OrderID`
FROM (`Orders` AS `o`
LEFT JOIN `Customers` AS `c` ON `o`.`CustomerID` = `c`.`CustomerID`)
LEFT JOIN `Orders` AS `o0` ON `c`.`CustomerID` = `o0`.`CustomerID`
WHERE (`o`.`OrderID` < 10800) AND (`c`.`CustomerID` IS NOT NULL AND `o0`.`CustomerID` IS NOT NULL)
ORDER BY `o`.`OrderID`
""");
    }
...
LEFT JOIN `Customers` AS `c` ON `o0`.`CustomerID` = `c`.`CustomerID`)
LEFT JOIN `Orders` AS `o1` ON `c`.`CustomerID` = `o1`.`CustomerID`
WHERE (`o`.`OrderID` = 10248) AND (`c`.`CustomerID` IS NOT NULL AND `o1`.`CustomerID` IS NOT NULL)
ORDER BY `o`.`OrderID`, `o`.`ProductID`
""");
    }
...
LEFT JOIN `Customers` AS `c` ON `o`.`CustomerID` = `c`.`CustomerID`)
LEFT JOIN `Orders` AS `o0` ON `c`.`CustomerID` = `o0`.`CustomerID`
WHERE (`o`.`CustomerID` LIKE 'F%') AND (`c`.`CustomerID` IS NOT NULL AND `o0`.`CustomerID` IS NOT NULL)
ORDER BY `o`.`OrderID`
""");
    }
...
FROM `Orders` AS `o`
LEFT JOIN `Customers` AS `c` ON `o`.`CustomerID` = `c`.`CustomerID`
ORDER BY NOT (`c`.`CustomerID` IS NOT NULL), IIF(`c`.`CustomerID` IS NOT NULL, `c`.`CustomerID`, ''), `o`.`OrderID`
""",
            //
            """
@p='2'
...
FROM (
    SELECT TOP @p `s0`.`OrderID`, `s0`.`c`, `s0`.`c0`
    FROM (
        SELECT `o`.`OrderID`, `c`.`CustomerID` IS NOT NULL AS `c`, IIF(`c`.`CustomerID` IS NOT NULL, `c`.`CustomerID`, '') AS `c0`
        FROM `Orders` AS `o`
        LEFT JOIN `Customers` AS `c` ON `o`.`CustomerID` = `c`.`CustomerID`
    ) AS `s0`
    ORDER BY NOT (`s0`.`c`), `s0`.`c0`, `s0`.`OrderID`
