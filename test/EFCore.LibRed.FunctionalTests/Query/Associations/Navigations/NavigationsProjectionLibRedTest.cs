...
public class NavigationsProjectionLibRedTest(NavigationsLibRedFixture fixture, ITestOutputHelper testOutputHelper)
    : NavigationsProjectionRelationalTestBase<NavigationsLibRedFixture>(fixture, testOutputHelper)
{
...
WHERE `a`.`RequiredNestedAssociateId` IS NOT NULL AND `n`.`Id` IS NOT NULL
""");
    }
...
INNER JOIN `AssociateType` AS `a` ON `r`.`RequiredAssociateId` = `a`.`Id`)
LEFT JOIN `NestedAssociateType` AS `n` ON `a`.`Id` = `n`.`CollectionAssociateId`
WHERE `a`.`Id` IS NOT NULL AND `n`.`CollectionAssociateId` IS NOT NULL
""");
    }
