...
public class AdHocQuerySplittingQueryLibRedTest(NonSharedFixture fixture) : AdHocQuerySplittingQueryTestBase(fixture)
{
    ...
    {
        new LibRedDbContextOptionsBuilder(optionsBuilder).UseQuerySplittingBehavior(splittingBehavior);
    ...
    {
        var extension = optionsBuilder.Options.FindExtension<LibRedOptionsExtension>();
        if (extension == null)
        {
            extension = new LibRedOptionsExtension();
        }
        else
        {
    ...
    {
        var testStore = LibRedTestStore.Create(StoreName);
...
""");
    ...
    public virtual async Task Using_AsSplitQuery_without_multiple_active_result_sets_works()
    {
        var contextFactory = await InitializeAsync<Context21355>(
            seed: c => c.SeedAsync(),
            createTestStore: () => LibRedTestStore.Create(StoreName));
