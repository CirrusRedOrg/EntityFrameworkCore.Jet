...
public class NavigationsCollectionLibRedTest(NavigationsLibRedFixture fixture, ITestOutputHelper testOutputHelper)
    : NavigationsCollectionRelationalTestBase<NavigationsLibRedFixture>(fixture, testOutputHelper)
{
...
WHERE ((
    ...
    FROM (
        ...
        FROM `AssociateType` AS `a`
        WHERE `r`.`Id` = `a`.`CollectionRootId`
    ) AS `a0`) = 2) AND (`a2`.`RequiredNestedAssociateId` IS NOT NULL AND `n2`.`Id` IS NOT NULL)
