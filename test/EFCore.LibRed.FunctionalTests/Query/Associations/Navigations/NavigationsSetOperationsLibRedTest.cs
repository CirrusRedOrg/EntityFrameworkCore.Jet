...
public class NavigationsSetOperationsLibRedTest(
    NavigationsLibRedFixture fixture,
    ITestOutputHelper testOutputHelper)
    : NavigationsSetOperationsRelationalTestBase<NavigationsLibRedFixture>(fixture, testOutputHelper)
{
...
LEFT JOIN `NestedAssociateType` AS `n2` ON `a2`.`RequiredNestedAssociateId` = `n2`.`Id`)
LEFT JOIN (
...
WHERE ((
    ...
    ) AS `u`) = 4) AND (`a2`.`RequiredNestedAssociateId` IS NOT NULL AND `n2`.`Id` IS NOT NULL)
...
LEFT JOIN `NestedAssociateType` AS `n4` ON `a`.`RequiredNestedAssociateId` = `n4`.`Id`)
LEFT JOIN (
...
WHERE ((
    ...
    ) AS `u`) = 4) AND (`a`.`RequiredNestedAssociateId` IS NOT NULL AND `n4`.`Id` IS NOT NULL)
ORDER BY `r`.`Id`, `s`.`Id`, `s`.`Id0`, `n8`.`Id`
""");
    }

    //skip this test as it crashes LibRed EF Core provider
    [Fact(Skip = "JET EF Core provider issue")]
