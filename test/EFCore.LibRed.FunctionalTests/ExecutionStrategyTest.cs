...
namespace EntityFrameworkCore.LibRed.FunctionalTests
{
    public class ExecutionStrategyTest : IClassFixture<ExecutionStrategyTest.ExecutionStrategyFixture>
    {
        ...
        private void Test_commit_failure(bool realFailure, Action<TestLibRedRetryingExecutionStrategy, ExecutionStrategyContext> execute)
        {
            CleanContext();
        ...
        private async Task Test_commit_failure_async(
            bool realFailure, Func<TestLibRedRetryingExecutionStrategy, ExecutionStrategyContext, Task> execute)
        {
            CleanContext();
        ...
        public void Handles_commit_failure_multiple_SaveChanges(bool realFailure)
        {
            ...
            using (var context2 = CreateContext())
            {
                ...
                new TestLibRedRetryingExecutionStrategy(context1).ExecuteInTransaction(
                    context1,
                    c1 =>
                    {
            ...
            bool realFailure, bool externalStrategy, bool openConnection, bool async)
        {
            ...
            using (var context = CreateContext())
            {
                ...
                if (async)
                {
                    if (externalStrategy)
                    {
                        await new TestLibRedRetryingExecutionStrategy(context).ExecuteInTransactionAsync(
                            context,
                            (c, ct) => c.SaveChangesAsync(false, ct),
                            (c, _) =>
                            {
                ...
                else
                {
                    if (externalStrategy)
                    {
                        new TestLibRedRetryingExecutionStrategy(context).ExecuteInTransaction(
                            context,
                            c => c.SaveChanges(false),
                            c =>
                            {
        ...
        public async Task Retries_query_on_execution_failure(bool externalStrategy, bool async)
        {
            ...
            using (var context = CreateContext())
            {
                ...
                if (async)
                {
                    if (externalStrategy)
                    {
                        list = await new TestLibRedRetryingExecutionStrategy(context)
                ...
                else
                {
                    if (externalStrategy)
                    {
                        list = new TestLibRedRetryingExecutionStrategy(context)
        ...
        public async Task Retries_OpenConnection_on_execution_failure(bool externalStrategy, bool async)
        {
            ...
            if (async)
            {
                if (externalStrategy)
                {
                    await new TestLibRedRetryingExecutionStrategy(context).ExecuteAsync(
                        context,
            ...
            else
            {
                if (externalStrategy)
                {
                    new TestLibRedRetryingExecutionStrategy(context).Execute(
                        context,
        ...
        public async Task Retries_BeginTransaction_on_execution_failure(bool async)
        {
            ...
            if (async)
            {
                var transaction = await new TestLibRedRetryingExecutionStrategy(context).ExecuteAsync(
                    context,
            ...
            {
                var transaction = new TestLibRedRetryingExecutionStrategy(context).Execute(
                    context,
        ...
        public void Verification_is_retried_using_same_retry_limit()
        {
            ...
            using (var context = CreateContext())
            {
                ...
                Assert.Throws<RetryLimitExceededException>(
                    () =>
                        new TestLibRedRetryingExecutionStrategy(context, TimeSpan.FromMilliseconds(100))
        ...
        public class ExecutionStrategyFixture : SharedStoreFixtureBase<DbContext>
        {
            ...
            public override DbContextOptionsBuilder AddOptions(DbContextOptionsBuilder builder)
            {
                ...
                new LibRedDbContextOptionsBuilder(options).MaxBatchSize(1);
                return options;
            }
