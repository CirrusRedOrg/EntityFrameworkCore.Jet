...
public class SortPushdownTests : TempDatabaseTest
{
    private static bool HasSortBelowJoin(PlanNode node, bool belowJoin = false)
        => node is SortNode && belowJoin
            || node.Children.Any(child => HasSortBelowJoin(child, belowJoin || node is JoinNode));

    private static bool ContainsJoin(PlanNode node)
        => node is JoinNode || node.Children.Any(ContainsJoin);

    private static bool HasSortAboveJoin(PlanNode node)
        => node is SortNode sort && ContainsJoin(sort.Input) || node.Children.Any(HasSortAboveJoin);

    private static QueryEngine Fresh()
    {
        string path = TemporaryDatabase.CopyPath(Path.Combine(AppContext.BaseDirectory, "Data", "Northwind.accdb"), "sortpd-");
    ...
    public void A_key_from_both_sides_stays_above_the_join()
    {
        ...
        QueryEngine e = Fresh();
        const string sql = "SELECT l.Id FROM L AS l, R AS r ORDER BY l.Grp, l.Id, r.Id DESC";
        Assert.True(HasSortAboveJoin(e.PlanFor(sql)));
        Assert.Equal([2, 2, 2, 4, 4, 4, 1, 1, 1, 3, 3, 3], Col(e, sql));
    }
    ...
    public void A_key_from_the_right_side_only_stays_above_the_join()
    {
        ...
        QueryEngine e = Fresh();
        const string sql = "SELECT r.Id FROM L AS l, R AS r WHERE l.Id = 1 ORDER BY r.Rk DESC, r.Id";
        Assert.True(HasSortAboveJoin(e.PlanFor(sql)));
        Assert.Equal([30, 10, 20], Col(e, sql));
    }
    ...
    public void An_unqualified_key_stays_above_the_join()
    {
        ...
        QueryEngine e = Fresh();
        const string sql = "SELECT l.Id FROM L AS l, R AS r ORDER BY Grp";
        Assert.True(HasSortAboveJoin(e.PlanFor(sql)));
        Assert.Equal([2, 2, 2, 4, 4, 4, 1, 1, 1, 3, 3, 3], Col(e, sql));
    }
    ...
    // The correctness tests above pass whether or not the sort is pushed, so pin its plan position directly.
    [Fact]
    public void The_pushdown_engages_on_a_three_way_cross_join_over_northwind()
    {
        using var temp = TemporaryDatabase.CopyOf(
            Path.Combine(AppContext.BaseDirectory, "Data", "Northwind.accdb"), "sortpdplan");
        ...
        Assert.True(HasSortBelowJoin(e.PlanFor(sql)), "expected the SortNode below a JoinNode");
        int rows = e.ExecuteQuery(sql).Rows.Count();
