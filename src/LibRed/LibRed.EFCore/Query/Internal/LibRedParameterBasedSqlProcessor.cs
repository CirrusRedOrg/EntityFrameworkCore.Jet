...
///     doing so can result in application failures when updating to a new Entity Framework Core release.
/// </remarks>
public class LibRedParameterBasedSqlProcessor(
    RelationalParameterBasedSqlProcessorDependencies dependencies,
    RelationalParameterBasedSqlProcessorParameters parameters,
    LibRedSqlMode sqlMode) : RelationalParameterBasedSqlProcessor(dependencies, parameters)
{
    /// <summary>
    ///     This is an internal API that supports the Entity Framework Core infrastructure and not subject to
    ///     the same compatibility standards as public APIs. It may be changed or removed without notice in
    ...
    public override Expression Process(Expression queryExpression, ParametersCacheDecorator parametersDecorator)
    {
        ...
        */
        ...
        // Both of these are Jet-dialect workarounds, so they run in compatible mode only.
        if (sqlMode == LibRedSqlMode.Compatible)
        {
            ...
            // In extended mode these constructs - ROW_NUMBER, CROSS/OUTER APPLY, EXCEPT, INTERSECT - are exactly what
            // LibRed is meant to attempt rather than reject.
