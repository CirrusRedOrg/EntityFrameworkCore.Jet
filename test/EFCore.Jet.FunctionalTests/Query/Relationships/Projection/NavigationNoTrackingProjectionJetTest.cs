...
public class NavigationNoTrackingProjectionJetTest
    : NavigationNoTrackingProjectionRelationalTestBase<NavigationRelationshipsJetFixture>
{
    public NavigationNoTrackingProjectionJetTest(NavigationRelationshipsJetFixture fixture, ITestOutputHelper testOutputHelper)
        : base(fixture)
    {
...
INNER JOIN `TrunkEntities` AS `t` ON `r`.`RequiredReferenceTrunkId` = `t`.`Id`)
LEFT JOIN `BranchEntities` AS `b` ON `t`.`Id` = `b`.`CollectionTrunkId`
WHERE `t`.`Id` IS NOT NULL AND `b`.`CollectionTrunkId` IS NOT NULL
""");
    }
