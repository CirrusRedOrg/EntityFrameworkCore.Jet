...
WHERE (`t`.`A` IS NOT NULL) AND (`t`.`A` LIKE 'A%')");
        }
...
WHERE (`t`.`Property` IS NOT NULL) AND (`t`.`Property` LIKE 'A%')");
        }
...
FROM (
    ...
    FROM `Orders` AS `o`
        ...
        public override async Task Comparing_to_fixed_string_parameter(bool isAsync)
        {
            ...
            AssertSql(
                $@"{AssertSqlHelper.Declaration("@__prefix_0='A' (Size = 255)")}
{AssertSqlHelper.Declaration("@__prefix_0='A' (Size = 255)")}
{AssertSqlHelper.Declaration("@__prefix_0='A' (Size = 255)")}
...
WHERE `o`.`CustomerID` = {AssertSqlHelper.Parameter("@_outer_CustomerID")}");
        ...
        public override async Task Join_take_count_works(bool isAsync)
        {
            ...
            AssertSql(
                $@"SELECT COUNT(*)
FROM (
    SELECT TOP 5 `o`.`OrderID`, `o`.`CustomerID`, `o`.`EmployeeID`, `o`.`OrderDate`, `t`.`CustomerID` AS `CustomerID0`, `t`.`Address`, `t`.`City`, `t`.`CompanyName`, `t`.`ContactName`, `t`.`ContactTitle`, `t`.`Country`, `t`.`Fax`, `t`.`Phone`, `t`.`PostalCode`, `t`.`Region`
    FROM `Orders` AS `o`
    INNER JOIN (
        SELECT `c`.`CustomerID`, `c`.`Address`, `c`.`City`, `c`.`CompanyName`, `c`.`ContactName`, `c`.`ContactTitle`, `c`.`Country`, `c`.`Fax`, `c`.`Phone`, `c`.`PostalCode`, `c`.`Region`
        FROM `Customers` AS `c`
