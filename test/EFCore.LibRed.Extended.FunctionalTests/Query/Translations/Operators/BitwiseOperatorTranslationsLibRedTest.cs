...
namespace EntityFrameworkCore.LibRed.Extended.FunctionalTests.Query.Translations.Operators;

public class BitwiseOperatorTranslationsLibRedTest : BitwiseOperatorTranslationsTestBase<BasicTypesQueryLibRedFixture>
{
    public BitwiseOperatorTranslationsLibRedTest(BasicTypesQueryLibRedFixture fixture, ITestOutputHelper testOutputHelper)
        : base(fixture)
    {
...
SELECT `b`.`Id`, `b`.`Bool`, `b`.`Byte`, `b`.`ByteArray`, `b`.`DateOnly`, `b`.`DateTime`, `b`.`DateTimeOffset`, `b`.`Decimal`, `b`.`Double`, `b`.`Enum`, `b`.`FlagsEnum`, `b`.`Float`, `b`.`Guid`, `b`.`Int`, `b`.`Long`, `b`.`Short`, `b`.`String`, `b`.`TimeOnly`, `b`.`TimeSpan`
FROM `BasicTypesEntities` AS `b`
WHERE (CLNG(`b`.`Int` BOR CLNG(`b`.`Short`)) BOR `b`.`Long`) = 7
""");
    }
