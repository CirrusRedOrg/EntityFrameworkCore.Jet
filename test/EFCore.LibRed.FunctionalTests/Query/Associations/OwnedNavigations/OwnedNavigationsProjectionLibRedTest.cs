...
public class OwnedNavigationsProjectionLibRedTest(OwnedNavigationsLibRedFixture fixture, ITestOutputHelper testOutputHelper)
    : OwnedNavigationsProjectionRelationalTestBase<OwnedNavigationsLibRedFixture>(fixture, testOutputHelper)
{
...
LEFT JOIN `RequiredRelated` AS `r0` ON `r`.`Id` = `r0`.`RootEntityId`)
LEFT JOIN `RequiredRelated_NestedCollection` AS `r1` ON `r0`.`RootEntityId` = `r1`.`AssociateTypeRootEntityId`
WHERE `r0`.`RootEntityId` IS NOT NULL AND `r1`.`AssociateTypeRootEntityId` IS NOT NULL
""");
        }
    }
...
LEFT JOIN `OptionalRelated` AS `o` ON `r`.`Id` = `o`.`RootEntityId`)
LEFT JOIN `OptionalRelated_NestedCollection` AS `o0` ON `o`.`RootEntityId` = `o0`.`AssociateTypeRootEntityId`
WHERE `o`.`RootEntityId` IS NOT NULL AND `o0`.`AssociateTypeRootEntityId` IS NOT NULL
""");
        }
    }
