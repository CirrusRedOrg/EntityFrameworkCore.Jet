...
{
    public class NorthwindKeylessEntitiesQueryLibRedTest : NorthwindKeylessEntitiesQueryRelationalTestBase<
        NorthwindQueryLibRedFixture<NoopModelCustomizer>>
    {
        public NorthwindKeylessEntitiesQueryLibRedTest(
...
""");
        ...
        public override async Task KeylessEntity_groupby(bool async)
        {
            ...
            AssertSql(
                """
SELECT `m`.`City` AS `Key`, COUNT(*) AS `Count`, COALESCE(SUM(LEN(`m`.`Address`)), 0) AS `Sum`
FROM (
    SELECT [c].[CustomerID], [c].[Address], [c].[City], [c].[CompanyName], [c].[ContactName], [c].[ContactTitle], [c].[Country], [c].[Fax], [c].[Phone], [c].[PostalCode], [c].[Region] FROM [Customers] AS [c]
) AS `m`
GROUP BY `m`.`City`
