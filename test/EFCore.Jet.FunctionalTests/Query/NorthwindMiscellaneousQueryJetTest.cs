...
{AssertSqlHelper.Declaration("@__prefix_0_1='A' (Size = 5)")}
{AssertSqlHelper.Declaration("@__prefix_0_1='A' (Size = 5)")}

SELECT `c`.`CustomerID`
FROM `Customers` AS `c`
WHERE {AssertSqlHelper.Parameter("@__prefix_0")} = '' OR LEFT(`c`.`CustomerID`, LEN({AssertSqlHelper.Parameter("@__prefix_0_1")})) = {AssertSqlHelper.Parameter("@__prefix_0_1")}");
        }
