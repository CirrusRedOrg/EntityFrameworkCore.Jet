...
public class NavigationReferenceNoTrackingProjectionJetTest
    : NavigationReferenceNoTrackingProjectionRelationalTestBase<NavigationRelationshipsJetFixture>
{
    public NavigationReferenceNoTrackingProjectionJetTest(NavigationRelationshipsJetFixture fixture, ITestOutputHelper testOutputHelper)
        : base(fixture)
    {
...
LEFT JOIN `BranchEntities` AS `b` ON `t`.`Id` = `b`.`Id`)
LEFT JOIN `LeafEntities` AS `l` ON `b`.`Id` = `l`.`Id`
WHERE `t`.`Id` IS NOT NULL AND `b`.`Id` IS NOT NULL AND `b`.`Id` IS NOT NULL AND `l`.`Id` IS NOT NULL
""");
    }
...
LEFT JOIN `BranchEntities` AS `b` ON `t`.`RequiredReferenceBranchId` = `b`.`Id`)
LEFT JOIN `LeafEntities` AS `l` ON `b`.`RequiredReferenceLeafId` = `l`.`Id`
WHERE `t`.`RequiredReferenceBranchId` IS NOT NULL AND `b`.`Id` IS NOT NULL AND `b`.`RequiredReferenceLeafId` IS NOT NULL AND `l`.`Id` IS NOT NULL
""");
    }
