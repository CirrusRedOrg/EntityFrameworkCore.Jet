...
public class TPCGearsOfWarQueryLibRedTest : TPCGearsOfWarQueryRelationalTestBase<TPCGearsOfWarQueryLibRedFixture>
{
#pragma warning disable IDE0060 // Remove unused parameter
    public TPCGearsOfWarQueryLibRedTest(TPCGearsOfWarQueryLibRedFixture fixture, ITestOutputHelper testOutputHelper)
...
""");
    ...
    public override async Task Select_inverted_nullable_boolean(bool async)
    {
        ...
        AssertSql(
            """
SELECT `l`.`Id`, NOT (`l`.`Eradicated`) AS `Alive`
...
WHERE IIF(`u`.`LeaderNickname` IS NULL, NULL, `u`.`LeaderNickname` LIKE '%us')
""");
    }
...
WHERE IIF(`u`.`LeaderNickname` IS NOT NULL, `u`.`LeaderNickname` LIKE '%us', NULL)
""");
    }
...
""");
    ...
    public override async Task Select_null_propagation_negative1(bool async)
    {
        ...
        AssertSql(
            """
SELECT IIF(`u`.`LeaderNickname` IS NOT NULL, LEN(`u`.`Nickname`) = 5, NULL)
FROM (
...
""");
    ...
    public override async Task Select_null_propagation_negative3(bool async)
    {
        ...
        AssertSql(
            """
SELECT `u0`.`Nickname`, IIF(`u0`.`Nickname` IS NOT NULL AND `u0`.`SquadId` IS NOT NULL, `u0`.`LeaderNickname` IS NOT NULL, NULL) AS `Condition`
FROM (
...
WHERE IIF(`u`.`HasSoulPatch` IS NULL, FALSE, `u`.`HasSoulPatch`)
""");
    }
...
WHERE (`t`.`Note` <> 'K.I.A.' OR `t`.`Note` IS NULL) AND IIF(`u`.`HasSoulPatch` IS NULL, FALSE, `u`.`HasSoulPatch`)
""");
    }
    ...
    public override async Task Coalesce_operator_in_projection_with_other_conditions(bool async)
    {
        ...
        AssertSql(
            """
SELECT (`t`.`Note` <> 'K.I.A.' OR `t`.`Note` IS NULL) AND IIF(`u`.`HasSoulPatch` IS NULL, FALSE, `u`.`HasSoulPatch`)
FROM `Tags` AS `t`
LEFT JOIN (
    SELECT `g`.`Nickname`, `g`.`SquadId`, `g`.`HasSoulPatch`
    FROM `Gears` AS `g`
...
WHERE (`t`.`Note` <> 'K.I.A.' OR `t`.`Note` IS NULL) AND `u`.`HasSoulPatch`
""");
    }
...
WHERE NOT (IIF(`u`.`HasSoulPatch`, TRUE, `u`.`HasSoulPatch`))
""");
    }
...
WHERE NOT (IIF(NOT (`u`.`HasSoulPatch`), FALSE, `u`.`HasSoulPatch`))
""");
    }
...
WHERE IIF(`u`.`HasSoulPatch`, TRUE, FALSE)
""");
    }
...
WHERE `u`.`HasSoulPatch` OR (`t`.`Note` LIKE '%Cole%')
""");
    }
    ...
    public override async Task Optional_navigation_type_compensation_works_with_binary_and_expression(bool async)
    {
        ...
        AssertSql(
            """
SELECT `u`.`HasSoulPatch` AND (`t`.`Note` LIKE '%Cole%') AND `t`.`Note` IS NOT NULL
FROM `Tags` AS `t`
LEFT JOIN (
    SELECT `g`.`Nickname`, `g`.`SquadId`, `g`.`HasSoulPatch`
    FROM `Gears` AS `g`
...
SELECT NOT EXISTS (
    ...
    WHERE (`t`.`Note` <> 'K.I.A.' OR `t`.`Note` IS NULL) AND NOT (`u`.`HasSoulPatch`))
...
LEFT JOIN `Weapons` AS `w` ON `u`.`HasSoulPatch`
""");
    }
...
WHERE `w`.`Id` <> 50 AND NOT (`u`.`HasSoulPatch`)
""");
    }
...
LEFT JOIN `Tags` AS `t` ON `u`.`Nickname` = `t`.`GearNickName` AND `u`.`SquadId` = `t`.`GearSquadId`
WHERE `u`.`FullName` <> 'Augustus Cole' AND NOT (`u`.`HasSoulPatch`)
ORDER BY `u`.`FullName`
""");
    }
...
WHERE `u`.`HasSoulPatch` AND NOT (`u0`.`HasSoulPatch`)
ORDER BY `u`.`FullName`
""");
    }
...
WHERE EXISTS (
    SELECT 1
    FROM `Weapons` AS `w`
    WHERE `u`.`FullName` = `w`.`OwnerFullName`) AND NOT (`u`.`HasSoulPatch`)
ORDER BY `u`.`Nickname`
""");
    }
...
) AS `s` ON `u0`.`Nickname` = `s`.`LeaderNickname` AND `u0`.`SquadId` = `s`.`LeaderSquadId`
...
WHERE EXISTS (
    ...
    WHERE `u`.`Nickname` = `u0`.`LeaderNickname` AND `u`.`SquadId` = `u0`.`LeaderSquadId`)
ORDER BY NOT (`u`.`HasSoulPatch`) DESC, `t`.`Note`, `u`.`Nickname`, `u`.`SquadId`, NOT (`s`.`IsAutomatic`), `s`.`Nickname` DESC
""");
    }
...
WHERE EXISTS (
    ...
    WHERE `u`.`Nickname` = `u0`.`LeaderNickname` AND `u`.`SquadId` = `u0`.`LeaderSquadId`)
ORDER BY NOT (`u`.`HasSoulPatch`) DESC, `t`.`Note`, `u`.`Nickname`, `u`.`SquadId`, `s`.`Id` DESC, `s`.`c`
""");
    }
...
WHERE EXISTS (
    ...
    WHERE `u`.`Nickname` = `u0`.`LeaderNickname` AND `u`.`SquadId` = `u0`.`LeaderSquadId`)
ORDER BY NOT (`u`.`HasSoulPatch`) DESC, `t`.`Note`, `u`.`Nickname`, `u`.`SquadId`, `s1`.`Rank`, `s1`.`Nickname`, `s1`.`SquadId`, NOT (`s1`.`IsAutomatic0`), `s1`.`Id`, `s1`.`Id0`, `s1`.`Nickname0`, `s1`.`SquadId0`, NOT (`s2`.`IsAutomatic`), `s2`.`Nickname` DESC
""");
    }
...
ORDER BY NOT (`u`.`HasSoulPatch`), `u`.`LeaderNickname`, `u`.`FullName`, `u`.`Nickname`, `u`.`SquadId`, `s1`.`FullName`, NOT (`s1`.`HasSoulPatch0`) DESC, `s1`.`Nickname`, `s1`.`SquadId`, NOT (`s1`.`IsAutomatic`), `s1`.`Name` DESC, `s1`.`Id`, `s1`.`Nickname0`
""");
    }
...
""");
    ...
    public override async Task Negated_bool_ternary_inside_anonymous_type_in_projection(bool async)
    {
        ...
        AssertSql(
            """
SELECT NOT (IIF(`u`.`HasSoulPatch`, TRUE, IIF(`u`.`HasSoulPatch` IS NULL, TRUE, `u`.`HasSoulPatch`))) AS `c`
FROM `Tags` AS `t`
LEFT JOIN (
    SELECT `g`.`Nickname`, `g`.`SquadId`, `g`.`HasSoulPatch`
    FROM `Gears` AS `g`
...
SELECT `s`.`Name`, IIF((
        ...
        WHERE `s`.`Id` = `u`.`SquadId` AND `u`.`HasSoulPatch`) IS NULL, 0, (
        SELECT TOP 1 `u`.`SquadId`
        FROM (
            SELECT `g`.`SquadId`, `g`.`HasSoulPatch`
            FROM `Gears` AS `g`
...
WHERE `s`.`Name` = 'Kilo' AND IIF((
        ...
        WHERE `s`.`Id` = `u`.`SquadId` AND `u`.`HasSoulPatch`)) <> 0
""");
    }
...
SELECT `s`.`Name`, IIF((
        ...
        WHERE `s`.`Id` = `u`.`SquadId` AND `u`.`HasSoulPatch`)) AS `Gear`
FROM `Squads` AS `s`
""");
    }
...
SELECT `s`.`Name`, IIF((
        ...
        WHERE `s`.`Id` = `u`.`SquadId` AND `u`.`HasSoulPatch`) IS NULL, FALSE, (
        SELECT TOP 1 TRUE
        FROM (
            SELECT `g`.`SquadId`, `g`.`HasSoulPatch`
            FROM `Gears` AS `g`
...
ORDER BY (
    ...
    WHERE `u`.`FullName` = `w`.`OwnerFullName` AND `w`.`IsAutomatic` = IIF((
            ...
            WHERE `u0`.`Nickname` = 'Marcus'))), `u`.`Nickname`, `u`.`SquadId`, `u2`.`Nickname`
""");
    }
...
WHERE `u`.`FullName` <> 'Dom' AND EXISTS (
    SELECT 1
    FROM `Weapons` AS `w`
    WHERE `u`.`FullName` = `w`.`OwnerFullName` AND `w`.`IsAutomatic`)
""");
    }
...
ORDER BY (
    SELECT TOP 1 `w`.`Name`
    FROM `Weapons` AS `w`
    WHERE `u`.`FullName` = `w`.`OwnerFullName` AND (`w`.`Name` LIKE '%Gnasher%')), `u`.`Nickname`, `u`.`SquadId`
""");
    }
...
ORDER BY `u0`.`FullName`, `u0`.`SquadId`
""");
    }
...
SELECT (`u`.`Rank` BOR (`u`.`Rank` BOR (@ranks BOR (`u`.`Rank` BOR @ranks)))) = @ranks
FROM (
    SELECT `g`.`Rank`
    FROM `Gears` AS `g`
    UNION ALL
...
WHERE IIF(`u`.`HasSoulPatch` = @prm, TRUE, FALSE)
""");
    }
...
WHERE IIF(`u`.`HasSoulPatch` = @prm, (
        SELECT TOP 1 `w`.`Name`
        FROM `Weapons` AS `w`
        WHERE `w`.`Id` = `u`.`SquadId`) = @prm2, FALSE)
""");
    }
...
WHERE (@prm BAND CLNG(CINT(`u`.`Rank`))) = `u`.`Rank`
""");
    }
...
WHERE `s`.`Name` = 'Delta' AND IIF((
        ...
        WHERE `s`.`Id` = `u`.`SquadId` AND `u`.`HasSoulPatch`
...
""");
    ...
    public override async Task Projecting_property_converted_to_nullable_into_element_init(bool async)
    {
        ...
        AssertSql(
            """
SELECT IIF(`t`.`GearNickName` IS NOT NULL, LEN(`u`.`Nickname`), NULL), IIF(`t`.`GearNickName` IS NOT NULL, `u`.`SquadId`, NULL), IIF(`t`.`GearNickName` IS NOT NULL, `u`.`SquadId`, NULL) + 1
FROM `Tags` AS `t`
LEFT JOIN (
    SELECT `g`.`Nickname`, `g`.`SquadId`
    FROM `Gears` AS `g`
...
WHERE IIF(`t`.`GearNickName` IS NOT NULL, `u`.`Nickname`, NULL) IS NOT NULL AND NOT (IIF(`t`.`GearNickName` IS NOT NULL, `u`.`HasSoulPatch`, NULL))
ORDER BY `t`.`Note`
""");
    }
...
LEFT JOIN `LocustHighCommands` AS `l2` ON IIF(`u`.`HighCommandId` IS NULL, NULL, CLNG(`u`.`HighCommandId`)) = `l2`.`Id`
""");
    }
...
FROM (
    SELECT TOP @p1 `s`.`Nickname`, `s`.`SquadId`, `s`.`AssignedCityName`, `s`.`CityOfBirthName`, `s`.`FullName`, `s`.`HasSoulPatch`, `s`.`LeaderNickname`, `s`.`LeaderSquadId`, `s`.`Rank`, `s`.`Discriminator`, `s`.`HasSoulPatch0`
    FROM (
        SELECT TOP @p + @p1 `u`.`Nickname`, `u`.`SquadId`, `u`.`AssignedCityName`, `u`.`CityOfBirthName`, `u`.`FullName`, `u`.`HasSoulPatch`, `u`.`LeaderNickname`, `u`.`LeaderSquadId`, `u`.`Rank`, `u`.`Discriminator`, `u1`.`HasSoulPatch` AS `HasSoulPatch0`
        FROM (
            SELECT `g`.`Nickname`, `g`.`SquadId`, `g`.`AssignedCityName`, `g`.`CityOfBirthName`, `g`.`FullName`, `g`.`HasSoulPatch`, `g`.`LeaderNickname`, `g`.`LeaderSquadId`, `g`.`Rank`, 'Gear' AS `Discriminator`
            FROM `Gears` AS `g`
            UNION ALL
        ...
        LEFT JOIN (
            SELECT MIN(LEN(`u0`.`Nickname`)) AS `c`, `u0`.`HasSoulPatch`
            FROM (
...
WHERE `u`.`HasSoulPatch` AND `u`.`HasSoulPatch` IN (@values1, @values2)
""");
    }
...
ORDER BY `u1`.`FullName`, `u1`.`SquadId`
""");
    }
...
WHERE IIF(EXISTS (
        SELECT 1
        FROM `Weapons` AS `w`
        WHERE `u`.`FullName` = `w`.`OwnerFullName`), 1, 0) IN (@numbers1, @numbers2)
""");
    }
...
WHERE IIF(`u`.`Rank` = @ranks1, @key, @key) IN (@keys1, @keys2)
""",
            //
            """
@ammoTypes1='1'
