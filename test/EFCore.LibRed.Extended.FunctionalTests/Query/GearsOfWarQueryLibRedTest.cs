...
{
    public class GearsOfWarQueryLibRedTest : GearsOfWarQueryRelationalTestBase<GearsOfWarQueryLibRedFixture>
    {
        // ReSharper disable once UnusedParameter.Local
#pragma warning disable IDE0060 // Remove unused parameter
        public GearsOfWarQueryLibRedTest(GearsOfWarQueryLibRedFixture fixture, ITestOutputHelper testOutputHelper)
...
@tags2='34c8d86e-a4ac-4be5-827f-584dda348a07'
@tags3='70534e05-782c-4052-8720-c2c54481ce5f'
@tags4='a8ad98f9-e023-4e2a-9a70-c2728455bd34'
@tags5='df36f493-463f-4123-83f9-6b135deeb7ba'
...
""");
        ...
        public override async Task Select_null_propagation_negative1(bool isAsync)
        {
            ...
            AssertSql(
                """
SELECT CASE
    WHEN `g`.`LeaderNickname` IS NOT NULL THEN LEN(`g`.`Nickname`) = 5
END
FROM `Gears` AS `g`
""");
        }
...
""");
        ...
        public override async Task Select_null_propagation_negative6(bool isAsync)
        {
            ...
            AssertSql(
                """
SELECT CASE
    WHEN `g`.`LeaderNickname` IS NOT NULL THEN FALSE
END
FROM `Gears` AS `g`
""");
        }
...
""");
        ...
        public override async Task Optional_Navigation_Null_Coalesce_To_Clr_Type(bool isAsync)
        {
            ...
            AssertSql(
                """
SELECT TOP 1 COALESCE(`w0`.`IsAutomatic`, FALSE) AS `IsAutomatic`
FROM `Weapons` AS `w`
...
WHERE `g`.`HasSoulPatch` AND COALESCE((
    SELECT TOP 1 `w`.`IsAutomatic`
    FROM `Weapons` AS `w`
    WHERE `g`.`FullName` = `w`.`OwnerFullName` AND (`w`.`Name` LIKE '%Lancer%')), FALSE)
ORDER BY `g`.`Nickname`, `g`.`SquadId`
""");
        }
...
FROM `Tags` AS `t`
LEFT JOIN `Gears` AS `g` ON `t`.`GearNickName` = `g`.`Nickname` AND `t`.`GearSquadId` = `g`.`SquadId`
WHERE COALESCE(`g`.`HasSoulPatch`, FALSE)
""");
        }
...
FROM `Tags` AS `t`
LEFT JOIN `Gears` AS `g` ON `t`.`GearNickName` = `g`.`Nickname` AND `t`.`GearSquadId` = `g`.`SquadId`
WHERE (`t`.`Note` <> 'K.I.A.' OR `t`.`Note` IS NULL) AND COALESCE(`g`.`HasSoulPatch`, FALSE)
""");
        }
        ...
        public override async Task Coalesce_operator_in_projection_with_other_conditions(bool isAsync)
        {
            ...
            AssertSql(
                """
SELECT (`t`.`Note` <> 'K.I.A.' OR `t`.`Note` IS NULL) AND COALESCE(`g`.`HasSoulPatch`, FALSE)
FROM `Tags` AS `t`
LEFT JOIN `Gears` AS `g` ON `t`.`GearNickName` = `g`.`Nickname` AND `t`.`GearSquadId` = `g`.`SquadId`
""");
        }
...
LEFT JOIN `Gears` AS `g` ON `t`.`GearNickName` = `g`.`Nickname` AND `t`.`GearSquadId` = `g`.`SquadId`
WHERE NOT (CASE
    WHEN NOT (`g`.`HasSoulPatch`) THEN FALSE
    ELSE `g`.`HasSoulPatch`
END)
""");
        }
...
FROM `Tags` AS `t`
LEFT JOIN `Gears` AS `g` ON `t`.`GearNickName` = `g`.`Nickname` AND `t`.`GearSquadId` = `g`.`SquadId`
WHERE `g`.`HasSoulPatch` OR (`t`.`Note` LIKE '%Cole%')
""");
        }
...
""");
        ...
        public override async Task Project_collection_navigation_with_inheritance1(bool isAsync)
        {
            ...
            AssertSql(
                """
SELECT `f`.`Id`, `l1`.`Name`, `l1`.`Discriminator`, `l1`.`LocustHordeId`, `l1`.`ThreatLevel`, `l1`.`ThreatLevelByte`, `l1`.`ThreatLevelNullableByte`, `l1`.`DefeatedByNickname`, `l1`.`DefeatedBySquadId`, `l1`.`HighCommandId`
FROM `Factions` AS `f`
LEFT JOIN (
    SELECT `l`.`Name`
    FROM `LocustLeaders` AS `l`
...
""");
        ...
        public override async Task Project_collection_navigation_with_inheritance2(bool isAsync)
        {
            ...
            AssertSql(
                """
SELECT `f`.`Id`, `g0`.`Nickname`, `g0`.`SquadId`, `g0`.`AssignedCityName`, `g0`.`CityOfBirthName`, `g0`.`Discriminator`, `g0`.`FullName`, `g0`.`HasSoulPatch`, `g0`.`LeaderNickname`, `g0`.`LeaderSquadId`, `g0`.`Rank`
FROM `Factions` AS `f`
LEFT JOIN (
    SELECT `l`.`Name`, `l`.`DefeatedByNickname`, `l`.`DefeatedBySquadId`
    FROM `LocustLeaders` AS `l`
...
""");
        ...
        public override async Task ToString_boolean_property_nullable(bool async)
        {
            ...
            AssertSql(
                """
SELECT CASE
    WHEN `f`.`Eradicated` = FALSE THEN 'False'
...
""");
        ...
        public override async Task ToString_boolean_computed_nullable(bool async)
        {
            ...
            AssertSql(
                """
SELECT CASE
    WHEN (`f`.`Eradicated` OR (`f`.`CommanderName` = 'Unknown' AND `f`.`CommanderName` IS NOT NULL)) = FALSE THEN 'False'
    WHEN `f`.`Eradicated` OR `f`.`CommanderName` = 'Unknown' THEN 'True'
    ELSE ''
END
FROM `Factions` AS `f`
""");
...
SELECT CASE `w`.`AmmunitionType`
    ...
    WHEN 2 THEN 'Shell'
    ELSE COALESCE((`w`.`AmmunitionType` & ''), '')
END
FROM `Weapons` AS `w`
""");
        }
...
""");
        ...
        public override async Task Multiple_orderby_with_navigation_expansion_on_one_of_the_order_bys_inside_subquery(bool isAsync)
        {
            ...
            AssertSql(
                """
SELECT `g`.`FullName`, `g`.`Nickname`, `g`.`SquadId`, `s`.`Id`, `s`.`AmmunitionType`, `s`.`IsAutomatic`, `s`.`Name`, `s`.`OwnerFullName`, `s`.`SynergyWithId`
FROM `Gears` AS `g`
LEFT JOIN `Tags` AS `t` ON `g`.`Nickname` = `t`.`GearNickName` AND `g`.`SquadId` = `t`.`GearSquadId`
LEFT JOIN `Gears` AS `g1` ON `t`.`GearNickName` = `g1`.`Nickname` AND `t`.`GearSquadId` = `g1`.`SquadId`
LEFT JOIN (
    SELECT `w`.`Id`, `w`.`AmmunitionType`, `w`.`IsAutomatic`, `w`.`Name`, `w`.`OwnerFullName`, `w`.`SynergyWithId`, `g2`.`Nickname`
    FROM `Weapons` AS `w`
...
WHERE `g`.`Discriminator` = 'Officer' AND EXISTS (
    ...
    FROM `Gears` AS `g0`
    WHERE `g`.`Nickname` = `g0`.`LeaderNickname` AND `g`.`SquadId` = `g0`.`LeaderSquadId`)
ORDER BY NOT (`g`.`HasSoulPatch`) DESC, `t`.`Note`, `g`.`Nickname`, `g`.`SquadId`, `s`.`Id` DESC, `s`.`c`
""");
        }
        ...
        public override async Task Correlated_collections_multiple_nested_complex_collections(bool isAsync)
        {
            ...
            AssertSql(
                """
SELECT `g`.`FullName`, `g`.`Nickname`, `g`.`SquadId`, `s1`.`FullName`, `s1`.`Nickname`, `s1`.`SquadId`, `s1`.`Id`, `s1`.`Name`, `s1`.`IsAutomatic`, `s1`.`Id0`, `s1`.`Nickname0`, `s1`.`HasSoulPatch`, `s1`.`SquadId0`, `s2`.`Id`, `s2`.`AmmunitionType`, `s2`.`IsAutomatic`, `s2`.`Name`, `s2`.`OwnerFullName`, `s2`.`SynergyWithId`
FROM `Gears` AS `g`
LEFT JOIN `Tags` AS `t` ON `g`.`Nickname` = `t`.`GearNickName` AND `g`.`SquadId` = `t`.`GearSquadId`
LEFT JOIN `Gears` AS `g1` ON `t`.`GearNickName` = `g1`.`Nickname` AND `t`.`GearSquadId` = `g1`.`SquadId`
LEFT JOIN (
    SELECT `g2`.`FullName`, `g2`.`Nickname`, `g2`.`SquadId`, `s0`.`Id`, `s0`.`Name`, `s0`.`IsAutomatic`, `s0`.`Id0`, `s0`.`Nickname` AS `Nickname0`, `s0`.`HasSoulPatch`, `s0`.`SquadId` AS `SquadId0`, `g2`.`Rank`, `s0`.`IsAutomatic0`, `g2`.`LeaderNickname`, `g2`.`LeaderSquadId`
    FROM `Gears` AS `g2`
    LEFT JOIN (
        SELECT `w`.`Id`, `w0`.`Name`, `w0`.`IsAutomatic`, `w0`.`Id` AS `Id0`, `g4`.`Nickname`, `g4`.`HasSoulPatch`, `g4`.`SquadId`, `w`.`IsAutomatic` AS `IsAutomatic0`, `w`.`OwnerFullName`
        FROM `Weapons` AS `w`
...
LEFT JOIN (
    SELECT `w1`.`Id`, `w1`.`AmmunitionType`, `w1`.`IsAutomatic`, `w1`.`Name`, `w1`.`OwnerFullName`, `w1`.`SynergyWithId`, `g5`.`Nickname`
    FROM `Weapons` AS `w1`
...
WHERE `g`.`Discriminator` = 'Officer'
ORDER BY `g`.`Nickname`, `g`.`SquadId`, `s`.`Nickname`, `s`.`SquadId`, `s`.`Id`
""");
        }
...
ORDER BY `s`.`Name`, `s`.`Id`, `g1`.`Nickname`, `g1`.`SquadId`
""");
        }
...
""");
        ...
        public override async Task Correlated_collections_deeply_nested_left_join(bool isAsync)
        {
            ...
            AssertSql(
                """
SELECT `t`.`Id`, `g`.`Nickname`, `g`.`SquadId`, `s0`.`Nickname`, `s0`.`SquadId`, `s0`.`Id`, `s0`.`AmmunitionType`, `s0`.`IsAutomatic`, `s0`.`Name`, `s0`.`OwnerFullName`, `s0`.`SynergyWithId`
FROM `Tags` AS `t`
LEFT JOIN `Gears` AS `g` ON `t`.`GearNickName` = `g`.`Nickname`
LEFT JOIN `Squads` AS `s` ON `g`.`SquadId` = `s`.`Id`
LEFT JOIN (
...
""");
        ...
        public override async Task Correlated_collections_complex_scenario1(bool isAsync)
        {
            ...
            AssertSql(
                """
SELECT `g`.`FullName`, `g`.`Nickname`, `g`.`SquadId`, `s0`.`Id`, `s0`.`Nickname`, `s0`.`HasSoulPatch`, `s0`.`SquadId`
FROM `Gears` AS `g`
LEFT JOIN (
    SELECT `w`.`Id`, `g1`.`Nickname`, `g1`.`HasSoulPatch`, `g1`.`SquadId`, `w`.`OwnerFullName`
    FROM `Weapons` AS `w`
...
""");
        ...
        public override async Task Correlated_collections_complex_scenario2(bool isAsync)
        {
            ...
            AssertSql(
                """
SELECT `g`.`FullName`, `g`.`Nickname`, `g`.`SquadId`, `s1`.`FullName`, `s1`.`Nickname`, `s1`.`SquadId`, `s1`.`Id`, `s1`.`Nickname0`, `s1`.`HasSoulPatch`, `s1`.`SquadId0`
FROM `Gears` AS `g`
LEFT JOIN (
    SELECT `g0`.`FullName`, `g0`.`Nickname`, `g0`.`SquadId`, `s0`.`Id`, `s0`.`Nickname` AS `Nickname0`, `s0`.`HasSoulPatch`, `s0`.`SquadId` AS `SquadId0`, `g0`.`LeaderNickname`, `g0`.`LeaderSquadId`
    FROM `Gears` AS `g0`
    LEFT JOIN (
        SELECT `w`.`Id`, `g2`.`Nickname`, `g2`.`HasSoulPatch`, `g2`.`SquadId`, `w`.`OwnerFullName`
        FROM `Weapons` AS `w`
...
FROM `Gears` AS `g`
LEFT JOIN (
    SELECT `g0`.`FullName`, `g0`.`Nickname`, `g0`.`SquadId`, `s0`.`Id`, `s0`.`Nickname` AS `Nickname0`, `s0`.`HasSoulPatch`, `s0`.`SquadId` AS `SquadId0`, `g0`.`HasSoulPatch` AS `HasSoulPatch0`, `s0`.`IsAutomatic`, `s0`.`Name`, `g0`.`LeaderNickname`, `g0`.`LeaderSquadId`
    FROM `Gears` AS `g0`
    LEFT JOIN (
        SELECT `w`.`Id`, `g2`.`Nickname`, `g2`.`HasSoulPatch`, `g2`.`SquadId`, `w`.`IsAutomatic`, `w`.`Name`, `w`.`OwnerFullName`
        FROM `Weapons` AS `w`
        LEFT JOIN `Gears` AS `g1` ON `w`.`OwnerFullName` = `g1`.`FullName`
        LEFT JOIN `Squads` AS `s` ON `g1`.`SquadId` = `s`.`Id`
        LEFT JOIN `Gears` AS `g2` ON `s`.`Id` = `g2`.`SquadId`
...
SELECT `s`.`Name`, `s`.`Discriminator`, `s`.`LocustHordeId`, `s`.`ThreatLevel`, `s`.`ThreatLevelByte`, `s`.`ThreatLevelNullableByte`, `s`.`DefeatedByNickname`, `s`.`DefeatedBySquadId`, `s`.`HighCommandId`, `s`.`Nickname`, `s`.`SquadId`, `s`.`AssignedCityName`, `s`.`CityOfBirthName`, `s`.`Discriminator0`, `s`.`FullName`, `s`.`HasSoulPatch`, `s`.`LeaderNickname`, `s`.`LeaderSquadId`, `s`.`Rank`, `w`.`Id`, `w`.`AmmunitionType`, `w`.`IsAutomatic`, `w`.`Name`, `w`.`OwnerFullName`, `w`.`SynergyWithId`
FROM (
...
WHERE `g`.`Discriminator` = 'Officer'
ORDER BY `g`.`Nickname`, `g`.`SquadId`, `s`.`Id`, `s`.`Nickname`, `s`.`SquadId`
""");
        }
...
SELECT NOT (CASE
    WHEN `g`.`HasSoulPatch` THEN TRUE
    ELSE COALESCE(`g`.`HasSoulPatch`, TRUE)
END) AS `c`
FROM `Tags` AS `t`
LEFT JOIN `Gears` AS `g` ON `t`.`GearNickName` = `g`.`Nickname` AND `t`.`GearSquadId` = `g`.`SquadId`
""");
...
SELECT `s`.`Name`, COALESCE((
    ...
    FROM `Gears` AS `g`
    WHERE `s`.`Id` = `g`.`SquadId` AND `g`.`HasSoulPatch`), 0) AS `SquadId`
FROM `Squads` AS `s`
WHERE `s`.`Name` = 'Kilo'
""");
        }
...
WHERE `s`.`Name` = 'Kilo' AND COALESCE((
    SELECT TOP 1 `g`.`SquadId`
    FROM `Gears` AS `g`
    WHERE `s`.`Id` = `g`.`SquadId` AND `g`.`HasSoulPatch`), 0) <> 0
""");
        }
...
SELECT `s`.`Name`, COALESCE((
    SELECT TOP 1 42
    FROM `Gears` AS `g`
    WHERE `s`.`Id` = `g`.`SquadId` AND `g`.`HasSoulPatch`), 0) AS `Gear`
FROM `Squads` AS `s`
""");
        }
...
SELECT `s`.`Name`, (
    SELECT TOP 1 'Foo'
    FROM `Gears` AS `g`
    WHERE `s`.`Id` = `g`.`SquadId` AND `g`.`HasSoulPatch`) AS `Gear`
FROM `Squads` AS `s`
""");
        }
...
SELECT `s`.`Name`, COALESCE((
    SELECT TOP 1 TRUE
    FROM `Gears` AS `g`
    WHERE `s`.`Id` = `g`.`SquadId` AND `g`.`HasSoulPatch`), FALSE) AS `Gear`
FROM `Squads` AS `s`
""");
        }
...
LEFT JOIN `Gears` AS `g0` ON `g`.`Nickname` = `g0`.`LeaderNickname` AND `g`.`SquadId` = `g0`.`LeaderSquadId`
WHERE `g`.`Discriminator` = 'Officer'
ORDER BY NOT (COALESCE((
    SELECT TOP 1 `w`.`IsAutomatic`
    FROM `Weapons` AS `w`
    WHERE `g`.`FullName` = `w`.`OwnerFullName`
    ORDER BY `w`.`Id`), FALSE)), `g`.`Nickname`, `g`.`SquadId`, `g0`.`Nickname`, `g0`.`SquadId`
""");
        }
...
ORDER BY (
    SELECT COUNT(*)
    FROM `Weapons` AS `w`
    WHERE `g`.`FullName` = `w`.`OwnerFullName`), `g`.`Nickname`, `g`.`SquadId`, `g1`.`Nickname`, `g1`.`SquadId`
""");
        }
...
ORDER BY (
    ...
    WHERE `g`.`FullName` = `w`.`OwnerFullName` AND `w`.`IsAutomatic` = COALESCE((
        ...
        FROM `Gears` AS `g0`
        WHERE `g0`.`Nickname` = 'Marcus'), FALSE)), `g`.`Nickname`, `g`.`SquadId`, `g2`.`Nickname`, `g2`.`SquadId`
""");
        }
...
SELECT COALESCE((
    SELECT TOP 1 `w`.`IsAutomatic`
    FROM `Weapons` AS `w`
    WHERE `g`.`FullName` = `w`.`OwnerFullName` AND `w`.`Name` = 'BFG'), FALSE)
FROM `Gears` AS `g`
WHERE `g`.`HasSoulPatch`
""");
        }
...
SELECT `w`.`Id`, `w`.`AmmunitionType`, `w`.`IsAutomatic`, `w`.`Name`, `w`.`OwnerFullName`, `w`.`SynergyWithId`
FROM `Weapons` AS `w`
ORDER BY NOT (`w`.`Name` = 'Marcus'' Lancer' AND `w`.`Name` IS NOT NULL), `w`.`Id`
""");
        }
...
FROM `Weapons` AS `w`
LEFT JOIN `Weapons` AS `w0` ON `w`.`SynergyWithId` = `w0`.`Id`
ORDER BY COALESCE(`w0`.`Name`, '') & (5 & ''), `w`.`Id`
""");
        }
...
FROM `Weapons` AS `w`
LEFT JOIN `Weapons` AS `w0` ON `w`.`SynergyWithId` = `w0`.`Id`
ORDER BY COALESCE(`w0`.`Name`, '') & 'Marcus'' Lancer', `w`.`Id`
""");
        }
        ...
        public override async Task String_concat_on_various_types(bool isAsync)
        {
            ...
            AssertSql(
                """
SELECT ('HasSoulPatch ' & (`g`.`HasSoulPatch` & '')) & ' HasSoulPatch' AS `HasSoulPatch`, ('Rank ' & (`g`.`Rank` & '')) & ' Rank' AS `Rank`, ('SquadId ' & (`g`.`SquadId` & '')) & ' SquadId' AS `SquadId`, ('Rating ' & COALESCE((`m`.`Rating` & ''), '')) & ' Rating' AS `Rating`, ('Timeline ' & (`m`.`Timeline` & '')) & ' Timeline' AS `Timeline`
FROM `Gears` AS `g`
...
LEFT JOIN (
    SELECT `s`.`Nickname`, `s`.`SquadId`, `s`.`AssignedCityName`, `s`.`CityOfBirthName`, `s`.`Discriminator`, `s`.`FullName`, `s`.`HasSoulPatch`, `s`.`LeaderNickname`, `s`.`LeaderSquadId`, `s`.`Rank`, `s`.`Name`, `s`.`Location`, `s`.`Nation`
    FROM (
        SELECT `g0`.`Nickname`, `g0`.`SquadId`, `g0`.`AssignedCityName`, `g0`.`CityOfBirthName`, `g0`.`Discriminator`, `g0`.`FullName`, `g0`.`HasSoulPatch`, `g0`.`LeaderNickname`, `g0`.`LeaderSquadId`, `g0`.`Rank`, `c`.`Name`, `c`.`Location`, `c`.`Nation`, ROW_NUMBER() OVER(PARTITION BY `g0`.`Rank` ORDER BY `g0`.`Nickname`, `g0`.`SquadId`) AS `row`
        FROM `Gears` AS `g0`
        INNER JOIN `Cities` AS `c` ON `g0`.`CityOfBirthName` = `c`.`Name`
        WHERE `g0`.`HasSoulPatch`
    ) AS `s`
...
SELECT `w`.`Id`, `w`.`AmmunitionType`, `w`.`IsAutomatic`, `w`.`Name`, `w`.`OwnerFullName`, `w`.`SynergyWithId`
FROM `Weapons` AS `w`
WHERE COALESCE(`w`.`SynergyWithId`, LEN(`w`.`Name`) + 42) > 10
""");
        }
...
ORDER BY (
    SELECT TOP 1 `w`.`Name`
    FROM `Weapons` AS `w`
    WHERE `g`.`FullName` = `w`.`OwnerFullName` AND (`w`.`Name` LIKE '%Gnasher%')), `g`.`Nickname`, `g`.`SquadId`, `w0`.`Id`
""");
        }
...
""");
        ...
        {
            var dto = LibRedTestHelpers.GetExpectedValue(new DateTimeOffset(599898024001234567, new TimeSpan(1, 30, 0)));
...
FROM `Missions` AS `m`
WHERE @start <= DATEVALUE(`m`.`Timeline`) AND `m`.`Timeline` < @end AND `m`.`Timeline` = @dates1
""");
        }
...
""");
        ...
        public override async Task Project_collection_navigation_nested_with_take_composite_key(bool isAsync)
        {
            ...
            AssertSql(
                """
SELECT `t`.`Id`, `g2`.`Nickname`, `g2`.`SquadId`, `g2`.`AssignedCityName`, `g2`.`CityOfBirthName`, `g2`.`Discriminator`, `g2`.`FullName`, `g2`.`HasSoulPatch`, `g2`.`LeaderNickname`, `g2`.`LeaderSquadId`, `g2`.`Rank`
FROM `Tags` AS `t`
LEFT JOIN `Gears` AS `g` ON `t`.`GearNickName` = `g`.`Nickname` AND `t`.`GearSquadId` = `g`.`SquadId`
LEFT JOIN (
    SELECT `g1`.`Nickname`, `g1`.`SquadId`, `g1`.`AssignedCityName`, `g1`.`CityOfBirthName`, `g1`.`Discriminator`, `g1`.`FullName`, `g1`.`HasSoulPatch`, `g1`.`LeaderNickname`, `g1`.`LeaderSquadId`, `g1`.`Rank`
...
WHERE `g`.`Discriminator` = 'Officer'
ORDER BY `t`.`Id`, `g2`.`Nickname`, `g2`.`SquadId`
""");
        }
...
""");
        ...
        public override async Task Nullable_bool_comparison_is_translated_to_server(bool isAsync)
        {
            ...
            AssertSql(
                """
SELECT `f`.`Eradicated` = TRUE AND `f`.`Eradicated` IS NOT NULL AS `IsEradicated`
FROM `Factions` AS `f`
""");
        }
...
""");
        ...
        public override async Task Accessing_property_of_optional_navigation_in_child_projection_works(bool isAsync)
        {
            ...
            AssertSql(
                """
SELECT `g`.`Nickname` IS NOT NULL AND `g`.`SquadId` IS NOT NULL, `t`.`Id`, `s`.`Nickname`, `s`.`Id`
FROM `Tags` AS `t`
LEFT JOIN `Gears` AS `g` ON `t`.`GearNickName` = `g`.`Nickname` AND `t`.`GearSquadId` = `g`.`SquadId`
LEFT JOIN (
    SELECT `g0`.`Nickname`, `w`.`Id`, `w`.`OwnerFullName`
    FROM `Weapons` AS `w`
    LEFT JOIN `Gears` AS `g0` ON `w`.`OwnerFullName` = `g0`.`FullName`
) AS `s` ON `g`.`FullName` = `s`.`OwnerFullName`
...
FROM `Squads` AS `s`
WHERE CASE
    WHEN ASCB(RIGHTB(`s`.`Banner5`, 1)) = 0 THEN LENB(`s`.`Banner5`) - 1
    ELSE LENB(`s`.`Banner5`)
END = 5
""");
        }
...
FROM `Gears` AS `g`
WHERE CASE
    WHEN `g`.`HasSoulPatch` = @prm THEN TRUE
    ELSE FALSE
END
""");
        }
...
SELECT `m`.`Id`, `m`.`CodeName`, `m`.`Date`, `m`.`Difficulty`, `m`.`Duration`, `m`.`Rating`, `m`.`Time`, `m`.`Timeline`
FROM `Missions` AS `m`
WHERE DATEVALUE(`m`.`Timeline`) >= @dateTimeOffset_Date
""");
        }
...
SELECT `g`.`Nickname`, COALESCE((
    ...
    ORDER BY `t1`.`Id`), #0100-01-01#) AS `invalidTagIssueDate`
FROM `Gears` AS `g`
LEFT JOIN `Tags` AS `t` ON `g`.`Nickname` = `t`.`GearNickName` AND `g`.`SquadId` = `t`.`GearSquadId`
...
""");
        ...
        public override async Task Projecting_property_converted_to_nullable_with_function_call2(bool async)
        {
            ...
            AssertSql(
                """
SELECT `t`.`Note`, MID(`t`.`Note`, 0 + 1, CASE
    WHEN `t`.`GearNickName` IS NOT NULL THEN `g`.`SquadId`
END) AS `Function`
FROM `Tags` AS `t`
LEFT JOIN `Gears` AS `g` ON `t`.`GearNickName` = `g`.`Nickname` AND `t`.`GearSquadId` = `g`.`SquadId`
...
""");
        ...
        public override async Task Projecting_property_converted_to_nullable_into_element_init(bool async)
        {
            ...
            AssertSql(
                """
SELECT CASE
    WHEN `t`.`GearNickName` IS NOT NULL THEN LEN(`g`.`Nickname`)
END, CASE
    WHEN `t`.`GearNickName` IS NOT NULL THEN `g`.`SquadId`
END, CASE
    WHEN `t`.`GearNickName` IS NOT NULL THEN `g`.`SquadId`
...
ORDER BY `g`.`Nickname`, `g`.`SquadId`, `w0`.`Name`, NOT (`w0`.`IsAutomatic`)
""");
        }
...
ORDER BY `g`.`Nickname`, `g`.`SquadId`, NOT (`w0`.`Key`)
""");
        }
...
ORDER BY `g`.`Nickname`, `g`.`SquadId`, NOT (`w0`.`IsAutomatic`), `w0`.`Name`
""");
        }
...
ORDER BY `g`.`Nickname`, `g`.`SquadId`, NOT (`s`.`HasSoulPatch`)
""");
        }
...
ORDER BY `s0`.`Id`, `s1`.`Nickname`, `s1`.`FullName`, NOT (`s1`.`HasSoulPatch`), `s1`.`Id0`
""");
        }
...
FROM (
    ...
    ) AS `g1` ON LEN(`g`.`Nickname`) = `g1`.`c`
    ORDER BY `g`.`Nickname`
    OFFSET @p ROWS FETCH NEXT @p1 ROWS ONLY
) AS `s`
LEFT JOIN `Weapons` AS `w` ON `s`.`FullName` = `w`.`OwnerFullName`
ORDER BY `s`.`Nickname`, `s`.`SquadId`, NOT (`s`.`HasSoulPatch0`), `w`.`Id`
""");
        }
...
""");
        ...
        public override async Task Set_operator_with_navigation_in_projection_groupby_aggregate(bool async)
        {
            ...
            AssertSql(
                """
SELECT `s`.`Name`, COALESCE(SUM(LEN(`c`.`Location`)), 0) AS `SumOfLengths`
FROM `Gears` AS `g`
INNER JOIN `Squads` AS `s` ON `g`.`SquadId` = `s`.`Id`
INNER JOIN `Cities` AS `c` ON `g`.`CityOfBirthName` = `c`.`Name`
WHERE 'Marcus' IN (
