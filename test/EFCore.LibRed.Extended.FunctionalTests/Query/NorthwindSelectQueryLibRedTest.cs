...
{
    public class NorthwindSelectQueryLibRedTest : NorthwindSelectQueryRelationalTestBase<NorthwindQueryLibRedFixture<NoopModelCustomizer>>
    {
        public NorthwindSelectQueryLibRedTest(
...
OUTER APPLY (
    SELECT TOP 2 `o`.`OrderID`, `o`.`CustomerID`, `o`.`EmployeeID`, `o`.`OrderDate`
    FROM `Orders` AS `o`
    WHERE LEN(`c`.`CustomerID`) >= LEN(`o`.`CustomerID`)
    ORDER BY `c`.`City`, `o`.`OrderID`
) AS `o0`
""");
        }
...
""");
        ...
        public override async Task Do_not_erase_projection_mapping_when_adding_single_projection(bool async)
        {
            ...
            AssertSql(
                """
SELECT `o`.`OrderID`, `o`.`CustomerID`, `o`.`EmployeeID`, `o`.`OrderDate`, `s`.`OrderID`, `s`.`ProductID`, `s`.`Discount`, `s`.`Quantity`, `s`.`UnitPrice`, `s`.`ProductID0`, `s`.`Discontinued`, `s`.`ProductName`, `s`.`SupplierID`, `s`.`UnitPrice0`, `s`.`UnitsInStock`, `s2`.`OrderID`, `s2`.`ProductID`, `s2`.`Discount`, `s2`.`Quantity`, `s2`.`UnitPrice`, `s2`.`ProductID0`, `s2`.`Discontinued`, `s2`.`ProductName`, `s2`.`SupplierID`, `s2`.`UnitPrice0`, `s2`.`UnitsInStock`, `s1`.`OrderID`, `s1`.`ProductID`, `s1`.`Discount`, `s1`.`Quantity`, `s1`.`UnitPrice`, `s1`.`ProductID0`, `s1`.`Discontinued`, `s1`.`ProductName`, `s1`.`SupplierID`, `s1`.`UnitPrice0`, `s1`.`UnitsInStock`
FROM `Orders` AS `o`
LEFT JOIN (
...
LEFT JOIN (
    ...
    FROM (
        SELECT `o1`.`OrderID`, `o1`.`ProductID`, `o1`.`Discount`, `o1`.`Quantity`, `o1`.`UnitPrice`, `p0`.`ProductID` AS `ProductID0`, `p0`.`Discontinued`, `p0`.`ProductName`, `p0`.`SupplierID`, `p0`.`UnitPrice` AS `UnitPrice0`, `p0`.`UnitsInStock`, ROW_NUMBER() OVER(PARTITION BY `o1`.`OrderID` ORDER BY `o1`.`OrderID`, `o1`.`ProductID`) AS `row`
...
FROM (
    SELECT DISTINCT `o`.`OrderID`, DATEPART('m', `o`.`OrderDate`) AS `Complex`
    FROM `Orders` AS `o`
) AS `o0`
OUTER APPLY (
...
ORDER BY `o0`.`OrderDate`, `o0`.`CustomerID`, `o2`.`Inner`
""");
        }
...
FROM (
    ...
    FROM (
        SELECT `o`.`OrderID`, DATEPART('m', `o`.`OrderDate`) AS `Complex`
        FROM `Orders` AS `o`
    ) AS `o0`
...
OUTER APPLY (
    ...
    WHERE `o`.`CustomerID` = `c`.`CustomerID` AND DATEPART('yyyy', `o`.`OrderDate`) = 1997
) AS `s`
WHERE `c`.`City` = 'London'
...
OUTER APPLY (
    ...
    FROM (
        ...
        FROM `Orders` AS `o`
        WHERE (`o`.`CustomerID` = `c0`.`City` OR (`o`.`CustomerID` IS NULL AND `c0`.`City` IS NULL)) AND DATEPART('yyyy', `o`.`OrderDate`) = 1997
    ) AS `o0`
    OUTER APPLY (
...
ORDER BY `o2`.`OrderID`, `s`.`OrderID` DESC
""");
        }
...
OUTER APPLY (
    SELECT CASE
        WHEN `o0`.`CustomerID` = `c0`.`CustomerID` OR (`o0`.`CustomerID` IS NULL AND `c0`.`CustomerID` IS NULL) THEN 'A'
        ELSE 'B'
...
FROM (
    ...
    FROM (
        SELECT `o`.`CustomerID`, DATEPART('m', `o`.`OrderDate`) AS `Complex`
        FROM `Orders` AS `o`
    ) AS `o0`
...
ORDER BY `c0`.`CustomerID`, `u`.`OrderID`
""");
        }
