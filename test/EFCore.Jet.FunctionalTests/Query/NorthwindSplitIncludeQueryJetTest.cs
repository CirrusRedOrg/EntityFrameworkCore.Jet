...
""");
    ...
    public override async Task Include_collection_order_by_collection_column(bool async)
    {
        ...
        AssertSql(
            """
SELECT TOP 1 `c0`.`c`, `c0`.`CustomerID`, `c0`.`Address`, `c0`.`City`, `c0`.`CompanyName`, `c0`.`ContactName`, `c0`.`ContactTitle`, `c0`.`Country`, `c0`.`Fax`, `c0`.`Phone`, `c0`.`PostalCode`, `c0`.`Region`
FROM (
    SELECT (
        SELECT TOP 1 `o`.`OrderDate`
        FROM `Orders` AS `o`
        WHERE `c`.`CustomerID` = `o`.`CustomerID`
        ORDER BY `o`.`OrderDate` DESC) AS `c`, `c`.`CustomerID`, `c`.`Address`, `c`.`City`, `c`.`CompanyName`, `c`.`ContactName`, `c`.`ContactTitle`, `c`.`Country`, `c`.`Fax`, `c`.`Phone`, `c`.`PostalCode`, `c`.`Region`
    FROM `Customers` AS `c`
...
""",
            //
            """
SELECT `c1`.`c`, `o0`.`OrderID`, `o0`.`CustomerID`, `o0`.`EmployeeID`, `o0`.`OrderDate`, `c1`.`CustomerID`
FROM (
    SELECT TOP 1 `c0`.`c`, `c0`.`CustomerID`
    FROM (
        SELECT (
            SELECT TOP 1 `o`.`OrderDate`
            FROM `Orders` AS `o`
            WHERE `c`.`CustomerID` = `o`.`CustomerID`
            ORDER BY `o`.`OrderDate` DESC) AS `c`, `c`.`CustomerID`
        FROM `Customers` AS `c`
        WHERE `c`.`CustomerID` LIKE 'W%'
    ) AS `c0`
    ORDER BY `c0`.`c` DESC
) AS `c1`
INNER JOIN `Orders` AS `o0` ON `c1`.`CustomerID` = `o0`.`CustomerID`
...
SELECT `o`.`OrderID`, `o`.`CustomerID`, `o`.`EmployeeID`, `o`.`OrderDate`, `c0`.`CustomerID`
FROM (
    SELECT TOP 10 `c`.`CustomerID`, `c`.`ContactTitle`
    FROM `Customers` AS `c`
...
""");
    ...
    public override async Task Include_duplicate_collection_result_operator(bool async)
    {
        ...
        AssertSql(
            """
SELECT TOP 1 `c1`.`CustomerID`, `c1`.`Address`, `c1`.`City`, `c1`.`CompanyName`, `c1`.`ContactName`, `c1`.`ContactTitle`, `c1`.`Country`, `c1`.`Fax`, `c1`.`Phone`, `c1`.`PostalCode`, `c1`.`Region`, `c2`.`CustomerID`, `c2`.`Address`, `c2`.`City`, `c2`.`CompanyName`, `c2`.`ContactName`, `c2`.`ContactTitle`, `c2`.`Country`, `c2`.`Fax`, `c2`.`Phone`, `c2`.`PostalCode`, `c2`.`Region`
FROM (
    SELECT TOP 2 `c`.`CustomerID`, `c`.`Address`, `c`.`City`, `c`.`CompanyName`, `c`.`ContactName`, `c`.`ContactTitle`, `c`.`Country`, `c`.`Fax`, `c`.`Phone`, `c`.`PostalCode`, `c`.`Region`
    FROM `Customers` AS `c`
    ORDER BY `c`.`CustomerID`
...
FROM (
    SELECT TOP 1 `c1`.`CustomerID`, `c2`.`CustomerID` AS `CustomerID0`
    FROM (
...
FROM (
    SELECT TOP 2 `o2`.`OrderID`, `o2`.`ProductID`
    FROM (
...
SELECT `o`.`OrderID`, `o`.`CustomerID`, `o`.`EmployeeID`, `o`.`OrderDate`, `c0`.`CustomerID`
FROM (
    SELECT TOP 10 `c`.`CustomerID`, `c`.`ContactName`
    FROM `Customers` AS `c`
...
ORDER BY NOT (`s`.`c`), `s`.`c0`, `s`.`OrderID`, `s`.`CustomerID`
""");
    }
...
INNER JOIN `Orders` AS `o0` ON `c1`.`CustomerID` = `o0`.`CustomerID`
ORDER BY `c1`.`c` DESC, `c1`.`CustomerID`, `o0`.`OrderID`
""",
            //
            """
SELECT `c1`.`c`, `o1`.`OrderID`, `o1`.`ProductID`, `o1`.`Discount`, `o1`.`Quantity`, `o1`.`UnitPrice`, `c1`.`CustomerID`, `o0`.`OrderID`
FROM ((
    SELECT TOP 1 `c0`.`c`, `c0`.`CustomerID`
    FROM (
        SELECT (
