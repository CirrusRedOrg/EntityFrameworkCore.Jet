        ...
        internal static string GetParameterValueToDisplay(DbParameter parameter)
        {
            ...
            else if (IsDateTime(parameter))
            {
                if (parameter.Value is TimeSpan)
                    return String.Format(CultureInfo.InvariantCulture, "#{0:c}#", parameter.Value);
                else
                    return String.Format(CultureInfo.InvariantCulture, "#{0:yyyy-MM-ddTHH:mm:ssZ}#", parameter.Value);
                ...
                return String.Format(CultureInfo.InvariantCulture, "{0}({1})",
                    Convert.ToInt32(parameter.Value, CultureInfo.InvariantCulture), parameter.Value);
            else
                return String.Format(CultureInfo.InvariantCulture, "{0}", parameter.Value);
