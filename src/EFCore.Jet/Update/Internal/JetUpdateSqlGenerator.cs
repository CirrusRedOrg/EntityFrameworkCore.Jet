                ...
                commandStringBuilder
                    ...
                    .AppendJoin(
                        operations, (sb, v) =>
                        {
                            ...
                            // Only a genuine AutoNumber (identity) column can be read back via @@identity. A key
                            // column that is store-generated by a DEFAULT (IsRead, so IsIdentityOperation is true)
                            // is NOT reflected by @@identity — that holds the last AutoNumber, possibly from another
                            // table — so `col = @@identity` would match nothing. For such a column, emit no
                            // condition: @@ROWCOUNT = 1 plus the other provided (non-read) key columns identify the
                            // just-inserted row, and its defaulted value is what we read back.
                            if (IsIdentityOperation(v) && isfirstkeycolumn
                                && v.Property?.GetValueGenerationStrategy() == JetValueGenerationStrategy.IdentityColumn)
                            {
