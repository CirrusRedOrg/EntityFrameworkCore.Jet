...
""");
        ...
        public async Task Where_equals_new_DateTime(bool async)
        {
            ...
            using var context = contextFactory.CreateContext();
            var query = context.Dates.Where(
                d => d.DateTime == new DateTime(1971, 9, 3, 12, 0, 10, 220));
...
SELECT `d`.`Id`, `d`.`DateTime`
FROM `Dates` AS `d`
WHERE `d`.`DateTime` = #1971-09-03 12:00:10#
""");
        }
        ...
        public async Task Where_contains_DateTime_literals(bool async)
        {
            ...
            using var context = contextFactory.CreateContext();
            var query = context.Dates.Where(
                d => dateTimes.Contains(d.DateTime));
...
SELECT `d`.`Id`, `d`.`DateTime`
FROM `Dates` AS `d`
WHERE `d`.`DateTime` IN (#1970-09-03 12:00:00#, #1971-09-03 12:00:10#, #1972-09-03 12:00:10#, #1973-09-03 12:00:10#, #1974-09-03 12:00:10#, #1975-09-03 12:00:10#, #1976-09-03 12:00:10#, #1977-09-03 12:00:10#, #1978-09-03 12:00:10#, #1979-09-03 12:00:10#, #1980-09-03 12:00:10#)
""");
        }
...
) AS `t0` ON `t`.`Id` = IIF(`t0`.`Exists` IS NOT NULL, `t0`.`Id`, NULL)
...
) AS `t2` ON `t`.`Id` = IIF(`t2`.`MaumarEntity11818_Exists` IS NOT NULL, `t2`.`Id`, NULL)
...
LEFT JOIN `City` AS `c` ON `b0`.`CityId` = `c`.`Id`)
INNER JOIN `MandatorSet` AS `m` ON `b`.`MandatorId` = `m`.`Id`
WHERE (`c`.`Name` = 'Leeds') AND (`b0`.`CityId` IS NOT NULL AND `c`.`Id` IS NOT NULL)
""");
            }
        }
...
LEFT JOIN (
    SELECT `d`.`Id`, `d0`.`Id` AS `Id0`, `d1`.`Id` AS `Id1`, IIF(IIF((
                SELECT MAX(`d2`.`GameNumber`)
                FROM `DbGame` AS `d2`
                WHERE `d1`.`Id` IS NOT NULL AND `d1`.`Id` = `d2`.`SeasonId`) IS NULL, 0, (
                SELECT MAX(`d2`.`GameNumber`)
                FROM `DbGame` AS `d2`
                WHERE `d1`.`Id` IS NOT NULL AND `d1`.`Id` = `d2`.`SeasonId`)) > 10, TRUE, FALSE) AS `IsPastTradeDeadline`, `d`.`DbTradeId`
...
""");
        ...
        public async Task Query_filter_with_pk_fk_optimization_Issue_13517()
        {
            ...
            using (var context = contextFactory.CreateContext())
            {
                ...
                AssertSql(
"""
SELECT TOP 2 `e`.`Id`, IIF(`t`.`Id` IS NULL, TRUE, FALSE), `t`.`Id`, `t`.`Public`, `e`.`RefEntityId`
FROM `Entities` AS `e`
LEFT JOIN (
...
""");
        ...
        public virtual async Task Can_configure_SingleQuery_at_context_level()
        {
            var contextFactory = await InitializeAsync<IssueContext21355>(
                seed: c => c.Seed(),
                onConfiguring: o => new JetDbContextOptionsBuilder(o).UseQuerySplittingBehavior(QuerySplittingBehavior.SingleQuery));
...
""");
        ...
        public virtual async Task Can_configure_SplitQuery_at_context_level()
        {
            var contextFactory = await InitializeAsync<IssueContext21355>(
                seed: c => c.Seed(),
                onConfiguring: o => new JetDbContextOptionsBuilder(o).UseQuerySplittingBehavior(QuerySplittingBehavior.SplitQuery));
...
""");
        ...
        public virtual async Task Using_AsSplitQuery_without_multiple_active_result_sets_works()
        {
            var contextFactory = await InitializeAsync<IssueContext21355>(
                seed: c => c.Seed(),
                createTestStore: () => JetTestStore.CreateInitialized(StoreName));
...
LEFT JOIN (
    ...
    LEFT JOIN `FourthFifthValueObjects` AS `f` ON IIF(`s`.`FourthValueObject_Value` IS NOT NULL, `s`.`Id`, NULL) = `f`.`SecondValueObjectId`)
    LEFT JOIN (
        ...
        LEFT JOIN `ThirdFifthValueObjects` AS `t3` ON IIF(`t0`.`FourthValueObject_Value` IS NOT NULL, `t0`.`Id`, NULL) = `t3`.`ThirdValueObjectId`
    ) AS `t1` ON `s`.`Id` = `t1`.`SecondValueObjectId`
) AS `t2` ON IIF(`t`.`FirstValueObject_Value` IS NOT NULL, `t`.`Id`, NULL) = `t2`.`AggregateId`
...
""");
        ...
        public virtual async Task Query_generates_correct_datetimeoffset_parameter_definition(int? fractionalSeconds, string postfix)
        {
            ...
            using (var context = contextFactory.CreateContext())
            {
                ...
                AssertSql(
"""
@__parameter_0='2021-11-12T13:14:15.1234567+10:00' (Nullable = false) (Size = 33)
...
""");
        ...
        protected override TestStore CreateTestStore()
            => JetTestStore.CreateInitialized(StoreName);
