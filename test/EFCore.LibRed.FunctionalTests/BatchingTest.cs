        ...
        : IClassFixture<BatchingTest.BatchingTestFixture>
    {
        ...
        public class BatchingTestFixture : SharedStoreFixtureBase<PoolableDbContext>
        {
                ...
                bool useConnectionString = false)
            {
                ...
                {
                    RelationalOptionsExtension extension = options.FindExtension<LibRedOptionsExtension>()
                        ?? new LibRedOptionsExtension();

                    extension = extension.WithConnection(null).WithConnectionString(((LibRedTestStore)TestStore).ConnectionString);
                    ((IDbContextOptionsBuilderInfrastructure)optionsBuilder).AddOrUpdateExtension(extension);
                }

                if (minBatchSize.HasValue)
                {
                    new LibRedDbContextOptionsBuilder(optionsBuilder).MinBatchSize(minBatchSize.Value);
                }

                if (maxBatchSize.HasValue)
                {
                    new LibRedDbContextOptionsBuilder(optionsBuilder).MinBatchSize(maxBatchSize.Value);
                }
