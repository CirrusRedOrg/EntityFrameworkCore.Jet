...
FROM [Alphabetical list of products] AS [a]");
        ...
        public override void QueryType_with_nav_defining_query()
        {
            ...
            AssertSql(
                @"@__ef_filter___searchTerm_0='A' (Nullable = false) (Size = 4000)
@__ef_filter___searchTerm_1='A' (Nullable = false) (Size = 4000)
...
WHERE (([t].[CompanyName] LIKE @__ef_filter___searchTerm_1 + '%' AND (LEFT([t].[CompanyName], LEN(@__ef_filter___searchTerm_1)) = @__ef_filter___searchTerm_1)) OR (@__ef_filter___searchTerm_1 = '')) AND ([t].[OrderCount] > 0)");
        }
