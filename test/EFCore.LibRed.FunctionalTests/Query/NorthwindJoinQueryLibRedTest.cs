...
{
    public class NorthwindJoinQueryLibRedTest : NorthwindJoinQueryRelationalTestBase<NorthwindQueryLibRedFixture<NoopModelCustomizer>>
    {
        public NorthwindJoinQueryLibRedTest(
...
WHERE `c`.`CustomerID` LIKE 'A%'
ORDER BY `c`.`CustomerID`, `s`.`OrderID0`, `s`.`OrderID`, `s`.`ProductID`
""");
        }
