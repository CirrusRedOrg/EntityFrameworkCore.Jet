...
| `0x18` | 4 | **AutoNumber increment** — the step added to `0x14` for each new id; default `1` (a plain `COUNTER`), and a custom `COUNTER(seed, increment)` / `AUTOINCREMENT(seed, increment)` / `INTEGER IDENTITY(seed, increment)` sets it (verified: `COUNTER(1000, 7)` → `0x18` = `7`, ids `1000, 1007, …`). It reads `1` on every table (autonumber or not) because that is the default increment — earlier notes and mdbtools mislabelled it a constant / "autonumber enable" flag; it is the increment value. The seed itself is not stored separately — it is recovered as `0x14 + increment` (correct on a freshly-created, un-inserted table). |
