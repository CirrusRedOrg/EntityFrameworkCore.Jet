...
LEFT JOIN `BaseEntities` AS `b0` ON `b`.`BaseParentId` = `b0`.`Id`)
LEFT JOIN `BaseEntities_OwnedCollectionOnBase` AS `b1` ON `b0`.`Id` = `b1`.`BaseInheritanceRelationshipEntityId`
WHERE `b0`.`Id` IS NOT NULL AND `b1`.`BaseInheritanceRelationshipEntityId` IS NOT NULL
ORDER BY `b`.`Id`, `b0`.`Id`
""",
                //
                """
...
WHERE `b0`.`Id` IS NOT NULL AND `b1`.`DerivedInheritanceRelationshipEntityId` IS NOT NULL
ORDER BY `b`.`Id`, `b0`.`Id`
""");
        }
...
LEFT JOIN `BaseEntities` AS `b` ON `c`.`ParentId` = `b`.`Id`)
LEFT JOIN `BaseEntities_OwnedCollectionOnBase` AS `b0` ON `b`.`Id` = `b0`.`BaseInheritanceRelationshipEntityId`
WHERE `b`.`Id` IS NOT NULL AND `b0`.`BaseInheritanceRelationshipEntityId` IS NOT NULL
ORDER BY `c`.`Id`, `b`.`Id`
""",
                //
                """
...
WHERE `b`.`Id` IS NOT NULL AND `b0`.`DerivedInheritanceRelationshipEntityId` IS NOT NULL
ORDER BY `c`.`Id`, `b`.`Id`
""");
        }
...
LEFT JOIN `BaseEntities_OwnedCollectionOnDerived` AS `b1` ON `t`.`Id` = `b1`.`DerivedInheritanceRelationshipEntityId`
WHERE `t`.`Id` IS NOT NULL AND `b1`.`DerivedInheritanceRelationshipEntityId` IS NOT NULL
ORDER BY `b`.`Id`, `t`.`Id`
""");
        }
...
WHERE `b0`.`Id` IS NOT NULL AND `n`.`ParentCollectionId` IS NOT NULL
ORDER BY `b`.`Id`, `b0`.`Id`
""");
        }
