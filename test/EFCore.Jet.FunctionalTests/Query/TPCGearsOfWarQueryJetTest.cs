...
WHERE (`u`.`Rank` BAND IIF((
        ...
        ORDER BY `u0`.`Nickname`, `u0`.`SquadId`) IS NULL, 0, (
        SELECT TOP 1 `u0`.`Rank`
        FROM (
            SELECT `g0`.`Nickname`, `g0`.`SquadId`, `g0`.`Rank`
            FROM `Gears` AS `g0`
        ...
        ORDER BY `u0`.`Nickname`, `u0`.`SquadId`))) = IIF((
        SELECT TOP 1 `u0`.`Rank`
        FROM (
            SELECT `g0`.`Nickname`, `g0`.`SquadId`, `g0`.`Rank`
            FROM `Gears` AS `g0`
...
WHERE (`u`.`Rank` BAND @__parameter_0) = @__parameter_0
""");
    }
...
WHERE IIF(`u`.`HasSoulPatch` = TRUE, TRUE, `u`.`HasSoulPatch`) <> TRUE
""");
    }
...
WHERE IIF(`u`.`HasSoulPatch` <> TRUE, FALSE, `u`.`HasSoulPatch`) <> TRUE
""");
    }
...
SELECT IIF(NOT EXISTS (
        ...
        WHERE (`t`.`Note` <> 'K.I.A.' OR `t`.`Note` IS NULL) AND `u`.`HasSoulPatch` <> TRUE), TRUE, FALSE)
...
WHERE `w`.`Id` <> 50 AND `u`.`HasSoulPatch` <> TRUE
""");
    }
...
LEFT JOIN `Tags` AS `t` ON `u`.`Nickname` = `t`.`GearNickName` AND `u`.`SquadId` = `t`.`GearSquadId`
WHERE `u`.`FullName` <> 'Augustus Cole' AND `u`.`HasSoulPatch` <> TRUE
ORDER BY `u`.`FullName`
""");
    }
...
WHERE `u`.`HasSoulPatch` = TRUE AND `u0`.`HasSoulPatch` <> TRUE
ORDER BY `u`.`FullName`
""");
    }
...
WHERE EXISTS (
    SELECT 1
    FROM `Weapons` AS `w`
    WHERE `u`.`FullName` = `w`.`OwnerFullName`) AND `u`.`HasSoulPatch` <> TRUE
ORDER BY `u`.`Nickname`
""");
    }
...
FROM (
    SELECT TOP 10 `u`.`Name`, `u`.`LocustHordeId`, `u`.`ThreatLevel`, `u`.`ThreatLevelByte`, `u`.`ThreatLevelNullableByte`, `u`.`DefeatedByNickname`, `u`.`DefeatedBySquadId`, `u`.`HighCommandId`, `u`.`Discriminator`, `u0`.`Nickname`, `u0`.`SquadId`, `u0`.`AssignedCityName`, `u0`.`CityOfBirthName`, `u0`.`FullName`, `u0`.`HasSoulPatch`, `u0`.`LeaderNickname`, `u0`.`LeaderSquadId`, `u0`.`Rank`, `u0`.`Discriminator` AS `Discriminator0`, `t`.`Id`, `t`.`Note`
    FROM ((
        SELECT `l`.`Name`, `l`.`LocustHordeId`, `l`.`ThreatLevel`, `l`.`ThreatLevelByte`, `l`.`ThreatLevelNullableByte`, NULL AS `DefeatedByNickname`, CVar(NULL) AS `DefeatedBySquadId`, CVar(NULL) AS `HighCommandId`, 'LocustLeader' AS `Discriminator`
        FROM `LocustLeaders` AS `l`
        UNION ALL
...
""");
    ...
    public override async Task Negated_bool_ternary_inside_anonymous_type_in_projection(bool async)
    {
        ...
        AssertSql(
            """
SELECT IIF(IIF(`u`.`HasSoulPatch` = TRUE, TRUE, IIF(`u`.`HasSoulPatch` IS NULL, TRUE, `u`.`HasSoulPatch`)) <> TRUE, TRUE, FALSE) AS `c`
FROM `Tags` AS `t`
LEFT JOIN (
    SELECT `g`.`Nickname`, `g`.`SquadId`, `g`.`HasSoulPatch`
    FROM `Gears` AS `g`
...
""");
    ...
    public override async Task Include_collection_OrderBy_aggregate(bool async)
    {
        ...
        AssertSql(
            """
SELECT `u1`.`c`, `u1`.`Nickname`, `u1`.`SquadId`, `u1`.`AssignedCityName`, `u1`.`CityOfBirthName`, `u1`.`FullName`, `u1`.`HasSoulPatch`, `u1`.`LeaderNickname`, `u1`.`LeaderSquadId`, `u1`.`Rank`, `u1`.`Discriminator`, `u0`.`Nickname`, `u0`.`SquadId`, `u0`.`AssignedCityName`, `u0`.`CityOfBirthName`, `u0`.`FullName`, `u0`.`HasSoulPatch`, `u0`.`LeaderNickname`, `u0`.`LeaderSquadId`, `u0`.`Rank`, `u0`.`Discriminator`
FROM (
    SELECT (
        SELECT COUNT(*)
        FROM `Weapons` AS `w`
        WHERE `u`.`FullName` = `w`.`OwnerFullName`) AS `c`, `u`.`Nickname`, `u`.`SquadId`, `u`.`AssignedCityName`, `u`.`CityOfBirthName`, `u`.`FullName`, `u`.`HasSoulPatch`, `u`.`LeaderNickname`, `u`.`LeaderSquadId`, `u`.`Rank`, `u`.`Discriminator`
    FROM (
        SELECT `o`.`Nickname`, `o`.`SquadId`, `o`.`AssignedCityName`, `o`.`CityOfBirthName`, `o`.`FullName`, `o`.`HasSoulPatch`, `o`.`LeaderNickname`, `o`.`LeaderSquadId`, `o`.`Rank`, 'Officer' AS `Discriminator`
        FROM `Officers` AS `o`
    ) AS `u`
...
LEFT JOIN (
    ...
    SELECT `o0`.`Nickname`, `o0`.`SquadId`, `o0`.`AssignedCityName`, `o0`.`CityOfBirthName`, `o0`.`FullName`, `o0`.`HasSoulPatch`, `o0`.`LeaderNickname`, `o0`.`LeaderSquadId`, `o0`.`Rank`, 'Officer' AS `Discriminator`
    FROM `Officers` AS `o0`
) AS `u0` ON `u1`.`Nickname` = `u0`.`LeaderNickname` AND `u1`.`SquadId` = `u0`.`LeaderSquadId`
ORDER BY `u1`.`c`, `u1`.`Nickname`, `u1`.`SquadId`, `u0`.`Nickname`
""");
    }
...
FROM (
    SELECT (
        ...
        FROM `Weapons` AS `w`
        WHERE `u`.`FullName` = `w`.`OwnerFullName`
        ORDER BY `w`.`Id`) AS `c`, `u`.`Nickname`, `u`.`SquadId`, `u`.`AssignedCityName`, `u`.`CityOfBirthName`, `u`.`FullName`, `u`.`HasSoulPatch`, `u`.`LeaderNickname`, `u`.`LeaderSquadId`, `u`.`Rank`, `u`.`Discriminator`
    FROM (
        SELECT `o`.`Nickname`, `o`.`SquadId`, `o`.`AssignedCityName`, `o`.`CityOfBirthName`, `o`.`FullName`, `o`.`HasSoulPatch`, `o`.`LeaderNickname`, `o`.`LeaderSquadId`, `o`.`Rank`, 'Officer' AS `Discriminator`
        FROM `Officers` AS `o`
    ) AS `u`
...
ORDER BY NOT (`u1`.`c`), `u1`.`Nickname`, `u1`.`SquadId`, `u0`.`Nickname`
""");
    }
...
""");
    ...
    public override async Task Correlated_collection_with_complex_OrderBy(bool async)
    {
        ...
        AssertSql(
            """
SELECT `u1`.`c`, `u1`.`Nickname`, `u1`.`SquadId`, `u2`.`Nickname`, `u2`.`SquadId`, `u2`.`AssignedCityName`, `u2`.`CityOfBirthName`, `u2`.`FullName`, `u2`.`HasSoulPatch`, `u2`.`LeaderNickname`, `u2`.`LeaderSquadId`, `u2`.`Rank`, `u2`.`Discriminator`
FROM (
    SELECT (
        SELECT COUNT(*)
        FROM `Weapons` AS `w`
        WHERE `u`.`FullName` = `w`.`OwnerFullName`) AS `c`, `u`.`Nickname`, `u`.`SquadId`
    FROM (
        SELECT `o`.`Nickname`, `o`.`SquadId`, `o`.`FullName`
        FROM `Officers` AS `o`
    ) AS `u`
...
LEFT JOIN (
    ...
    WHERE `u0`.`HasSoulPatch` <> TRUE
) AS `u2` ON `u1`.`Nickname` = `u2`.`LeaderNickname` AND `u1`.`SquadId` = `u2`.`LeaderSquadId`
ORDER BY `u1`.`c`, `u1`.`Nickname`, `u1`.`SquadId`, `u2`.`Nickname`
""");
    }
    ...
    public override async Task Correlated_collection_with_very_complex_order_by(bool async)
    {
        ...
        AssertSql(
            """
SELECT `u2`.`c`, `u2`.`Nickname`, `u2`.`SquadId`, `u3`.`Nickname`, `u3`.`SquadId`, `u3`.`AssignedCityName`, `u3`.`CityOfBirthName`, `u3`.`FullName`, `u3`.`HasSoulPatch`, `u3`.`LeaderNickname`, `u3`.`LeaderSquadId`, `u3`.`Rank`, `u3`.`Discriminator`
FROM (
    SELECT (
        SELECT COUNT(*)
        FROM `Weapons` AS `w`
        WHERE `u`.`FullName` = `w`.`OwnerFullName` AND `w`.`IsAutomatic` = IIF((
                ...
                WHERE `u0`.`Nickname` = 'Marcus'))) AS `c`, `u`.`Nickname`, `u`.`SquadId`
    FROM (
        SELECT `o`.`Nickname`, `o`.`SquadId`, `o`.`FullName`
        FROM `Officers` AS `o`
    ) AS `u`
...
LEFT JOIN (
    ...
    WHERE `u1`.`HasSoulPatch` <> TRUE
) AS `u3` ON `u2`.`Nickname` = `u3`.`LeaderNickname` AND `u2`.`SquadId` = `u3`.`LeaderSquadId`
ORDER BY `u2`.`c`, `u2`.`Nickname`, `u2`.`SquadId`, `u3`.`Nickname`
""");
    }
...
ORDER BY NOT (IIF(IIF(`u`.`LeaderNickname` IS NOT NULL, IIF(IIF(LEN(`u`.`Nickname`) IS NULL, NULL, CLNG(LEN(`u`.`Nickname`))) = 5, TRUE, FALSE), NULL) IS NOT NULL, TRUE, FALSE))
""");
    }
...
""");
    ...
    public override async Task Include_with_complex_order_by(bool async)
    {
        ...
        AssertSql(
            """
SELECT `u0`.`c`, `u0`.`Nickname`, `u0`.`SquadId`, `u0`.`AssignedCityName`, `u0`.`CityOfBirthName`, `u0`.`FullName`, `u0`.`HasSoulPatch`, `u0`.`LeaderNickname`, `u0`.`LeaderSquadId`, `u0`.`Rank`, `u0`.`Discriminator`, `w0`.`Id`, `w0`.`AmmunitionType`, `w0`.`IsAutomatic`, `w0`.`Name`, `w0`.`OwnerFullName`, `w0`.`SynergyWithId`
FROM (
    SELECT (
        SELECT TOP 1 `w`.`Name`
        FROM `Weapons` AS `w`
        WHERE `u`.`FullName` = `w`.`OwnerFullName` AND (`w`.`Name` LIKE '%Gnasher%')) AS `c`, `u`.`Nickname`, `u`.`SquadId`, `u`.`AssignedCityName`, `u`.`CityOfBirthName`, `u`.`FullName`, `u`.`HasSoulPatch`, `u`.`LeaderNickname`, `u`.`LeaderSquadId`, `u`.`Rank`, `u`.`Discriminator`
    FROM (
        SELECT `g`.`Nickname`, `g`.`SquadId`, `g`.`AssignedCityName`, `g`.`CityOfBirthName`, `g`.`FullName`, `g`.`HasSoulPatch`, `g`.`LeaderNickname`, `g`.`LeaderSquadId`, `g`.`Rank`, 'Gear' AS `Discriminator`
        FROM `Gears` AS `g`
        UNION ALL
...
LEFT JOIN `Weapons` AS `w0` ON `u0`.`FullName` = `w0`.`OwnerFullName`
ORDER BY `u0`.`c`, `u0`.`Nickname`, `u0`.`SquadId`
""");
    }
...
WHERE (`u`.`Rank` BAND @__rank_0) = @__rank_0
""",
            //
            """
SELECT `u`.`Nickname`, `u`.`SquadId`, `u`.`AssignedCityName`, `u`.`CityOfBirthName`, `u`.`FullName`, `u`.`HasSoulPatch`, `u`.`LeaderNickname`, `u`.`LeaderSquadId`, `u`.`Rank`, `u`.`Discriminator`
...
WHERE (`u`.`Rank` BOR @__rank_0) <> @__rank_0
""",
            //
            """
SELECT `u`.`Nickname`, `u`.`SquadId`, `u`.`AssignedCityName`, `u`.`CityOfBirthName`, `u`.`FullName`, `u`.`HasSoulPatch`, `u`.`LeaderNickname`, `u`.`LeaderSquadId`, `u`.`Rank`, `u`.`Discriminator`
...
SELECT IIF((`u`.`Rank` BOR @__ranks_0) = @__ranks_0, TRUE, FALSE)
FROM (
    SELECT `g`.`Rank`
    FROM `Gears` AS `g`
    UNION ALL
...
SELECT IIF((`u`.`Rank` BOR (`u`.`Rank` BOR (@__ranks_0 BOR (`u`.`Rank` BOR @__ranks_0)))) = @__ranks_0, TRUE, FALSE)
FROM (
    SELECT `g`.`Rank`
    FROM `Gears` AS `g`
    UNION ALL
...
WHERE IIF(`u`.`HasSoulPatch` = @__prm_0, TRUE, FALSE) = TRUE
""");
    }
...
WHERE IIF(`u`.`HasSoulPatch` = @__prm_0, IIF((
            SELECT TOP 1 `w`.`Name`
            FROM `Weapons` AS `w`
            WHERE `w`.`Id` = `u`.`SquadId`) = @__prm2_1 AND (
            SELECT TOP 1 `w`.`Name`
            FROM `Weapons` AS `w`
            WHERE `w`.`Id` = `u`.`SquadId`) IS NOT NULL, TRUE, FALSE), FALSE) = TRUE
""");
    }
...
WHERE INSTR(1, STRCONV(`s`.`Banner`, 64), CHR(`u`.`ThreatLevelByte`), 0) > 0
""");
    }
...
WHERE (@__prm_0 BAND `u`.`Rank`) = `u`.`Rank`
""");
    }
...
WHERE (@__prm_0 BAND CLNG(`u`.`Rank`)) = CLNG(`u`.`Rank`)
""");
    }
...
WHERE IIF(`t`.`GearNickName` IS NOT NULL, `u`.`Nickname`, NULL) IS NOT NULL AND IIF(`t`.`GearNickName` IS NOT NULL, `u`.`HasSoulPatch`, NULL) <> TRUE
ORDER BY `t`.`Note`
""");
    }
...
FROM (
    SELECT TOP 10 `s`.`Nickname`, `s`.`SquadId`, `s`.`AssignedCityName`, `s`.`CityOfBirthName`, `s`.`FullName`, `s`.`HasSoulPatch`, `s`.`LeaderNickname`, `s`.`LeaderSquadId`, `s`.`Rank`, `s`.`Discriminator`, `s`.`HasSoulPatch0`
    FROM (
        SELECT TOP 10 `u`.`Nickname`, `u`.`SquadId`, `u`.`AssignedCityName`, `u`.`CityOfBirthName`, `u`.`FullName`, `u`.`HasSoulPatch`, `u`.`LeaderNickname`, `u`.`LeaderSquadId`, `u`.`Rank`, `u`.`Discriminator`, `u1`.`HasSoulPatch` AS `HasSoulPatch0`
        FROM (
            SELECT `g`.`Nickname`, `g`.`SquadId`, `g`.`AssignedCityName`, `g`.`CityOfBirthName`, `g`.`FullName`, `g`.`HasSoulPatch`, `g`.`LeaderNickname`, `g`.`LeaderSquadId`, `g`.`Rank`, 'Gear' AS `Discriminator`
            FROM `Gears` AS `g`
            UNION ALL
...
SELECT TOP 1 `u`.`Rank` BAND @__value_1
FROM (
    SELECT `g`.`Nickname`, `g`.`Rank`
    FROM `Gears` AS `g`
    UNION ALL
...
FROM (
    ...
    FROM (
        SELECT TOP 1 `u`.`Nickname`, `u`.`SquadId`, `u`.`AssignedCityName`, `u`.`CityOfBirthName`, `u`.`FullName`, `u`.`HasSoulPatch`, `u`.`LeaderNickname`, `u`.`LeaderSquadId`, `u`.`Rank`, `u`.`Discriminator`
        FROM (
            SELECT `g`.`Nickname`, `g`.`SquadId`, `g`.`AssignedCityName`, `g`.`CityOfBirthName`, `g`.`FullName`, `g`.`HasSoulPatch`, `g`.`LeaderNickname`, `g`.`LeaderSquadId`, `g`.`Rank`, 'Gear' AS `Discriminator`
            FROM `Gears` AS `g`
            UNION ALL
...
FROM (
    SELECT TOP 1 `u0`.`Nickname`, `u0`.`SquadId`, `u0`.`AssignedCityName`, `u0`.`CityOfBirthName`, `u0`.`FullName`, `u0`.`HasSoulPatch`, `u0`.`LeaderNickname`, `u0`.`LeaderSquadId`, `u0`.`Rank`, `u0`.`Discriminator`
    FROM (
        SELECT TOP 2 `u`.`Nickname`, `u`.`SquadId`, `u`.`AssignedCityName`, `u`.`CityOfBirthName`, `u`.`FullName`, `u`.`HasSoulPatch`, `u`.`LeaderNickname`, `u`.`LeaderSquadId`, `u`.`Rank`, `u`.`Discriminator`
        FROM (
            SELECT `g`.`Nickname`, `g`.`SquadId`, `g`.`AssignedCityName`, `g`.`CityOfBirthName`, `g`.`FullName`, `g`.`HasSoulPatch`, `g`.`LeaderNickname`, `g`.`LeaderSquadId`, `g`.`Rank`, 'Gear' AS `Discriminator`
            FROM `Gears` AS `g`
            UNION ALL
...
FROM (
    SELECT TOP 1 `u0`.`Nickname`, `u0`.`SquadId`, `u0`.`AssignedCityName`, `u0`.`CityOfBirthName`, `u0`.`FullName`, `u0`.`HasSoulPatch`, `u0`.`LeaderNickname`, `u0`.`LeaderSquadId`, `u0`.`Rank`, `u0`.`Discriminator`
    FROM (
        SELECT TOP 3 `u`.`Nickname`, `u`.`SquadId`, `u`.`AssignedCityName`, `u`.`CityOfBirthName`, `u`.`FullName`, `u`.`HasSoulPatch`, `u`.`LeaderNickname`, `u`.`LeaderSquadId`, `u`.`Rank`, `u`.`Discriminator`
        FROM (
            SELECT `g`.`Nickname`, `g`.`SquadId`, `g`.`AssignedCityName`, `g`.`CityOfBirthName`, `g`.`FullName`, `g`.`HasSoulPatch`, `g`.`LeaderNickname`, `g`.`LeaderSquadId`, `g`.`Rank`, 'Gear' AS `Discriminator`
            FROM `Gears` AS `g`
            UNION ALL
...
WHERE NOT EXISTS (
    ...
    FROM `SquadMissions` AS `s0`
    INNER JOIN `Missions` AS `m` ON `s0`.`MissionId` = `m`.`Id`
    WHERE `s`.`Id` = `s0`.`SquadId` AND @__unixEpochSeconds_0 = DATEDIFF('s', CDATE('1970-01-01 00:00:00'), `m`.`Timeline`))
...
WHERE IIF(`u`.`Rank` = 1, @__key_1, @__key_1) IN ('{0a47bcb7-a1cb-4345-8944-c58f82d6aac7}', '{5f221fb9-66f4-442a-92c9-d97ed5989cc7}')
""",
            //
            """
@__key_1='5f221fb9-66f4-442a-92c9-d97ed5989cc7'
