...
#nullable disable
public class ManyToManyLoadLibRedTest : ManyToManyLoadTestBase<ManyToManyLoadLibRedTest.ManyToManyLoadLibRedFixture>
{
    public ManyToManyLoadLibRedTest(ManyToManyLoadLibRedFixture fixture, ITestOutputHelper testOutputHelper)
        : base(fixture)
    {
...
LEFT JOIN (
    ...
    WHERE `e1`.`Id` = {AssertSqlHelper.Parameter("@p")}
) AS `s0` ON `s`.`Id` = `s0`.`TwoId`
WHERE `e`.`Id` = {AssertSqlHelper.Parameter("@p")}
...
WHERE (`e`.`Id` = @p) AND (`s`.`Id` IS NOT NULL AND `s1`.`Id0` IS NOT NULL)
...
""");
    ...
    public class ManyToManyLoadLibRedFixture : ManyToManyLoadFixtureBase, ITestSqlLoggerFactory
    {
