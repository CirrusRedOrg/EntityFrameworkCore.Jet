...
namespace EntityFrameworkCore.LibRed.Migrations.Internal
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </remarks>
    public class LibRedHistoryRepository(HistoryRepositoryDependencies dependencies) : HistoryRepository(dependencies)
    {
...
""");
        ...
        ///     </para>
        ///     <para>
        ///         Unlike Jet/ACE, LibRed accepts a FROM-less SELECT, so the source needs no DUAL stand-in and no
        ///         <c>(SELECT COUNT(*) FROM …)</c> wrapper around it.
        ///     </para>
        ///     <para>
